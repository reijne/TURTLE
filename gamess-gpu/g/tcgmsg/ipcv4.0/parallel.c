/* $Header: /c/qcg/cvs/psh/GAMESS-UK/g/tcgmsg/ipcv4.0/parallel.c,v 1.10 2007-10-30 11:54:29 jmht Exp $ */
 ...
 */
{
  ...
  if (pid == 0) {
    ...
    if (strcmp_host(remote_hostname, local_hostname) != 0) {
      argv2[0      ] = "rsh";
      argv2[1      ] = remote_hostname;
      argv2[2      ] = "-l";
      argv2[3      ] = remote_username;

      /*  local (gamess-uk) change, fouled up on Delft with SSH */
      /*    see HvD for more details */
      /* argv2[4      ] = "-n"; */
      argv2[4      ] = "  ";
...
int strcmp_host(char *remote_hostname, 
		char *local_hostname){
  ...
  l1 = strlen(remote_hostname);
  l2 = strlen(local_hostname);

  if(l1 > l2 && remote_hostname[l2] == '.'){
    res = strncmp(remote_hostname,local_hostname,l2);
  }else if(l1 < l2 && local_hostname[l1] == '.'){
    res = strncmp(local_hostname,remote_hostname,l1);
  }else{
    res = strcmp(remote_hostname,local_hostname);
  }
}
