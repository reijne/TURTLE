/* $Header: /c/qcg/cvs/psh/GAMESS-UK/g/tcgmsg-mpi/pbeginf.c,v 1.3 2007-10-30 11:57:22 jmht Exp $ */
...
#elif !defined(HPUX)
#if defined(GFS)
    /* CMR: gfortran doesn't come with iargc_, getarg_ */
	int	argc= _gfortran_iargc();
#else
    extern int iargc_();
    extern void getarg_();
    int argc = iargc_() + 1;
...
#      elif  defined(__crayx1)
          ...
          len=(int)ilen;
#      elif defined(GFS)
          /* CMR: gfortran doesn't come with iargc_, getarg_ picked this name with nm 'a.out'*/
          int ii=i;
		  _gfortran_getarg_i4(&ii,arg);
          len=strlen(arg);
