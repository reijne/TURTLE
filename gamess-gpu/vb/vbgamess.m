...
c     endif
      ...
      subroutine gatdotx(n,supg,map,w,val)
      implicit REAL (a-h,o-z), integer (i-n)
INCLUDE(common/tractlt)
      dimension supg(0:n2int),map(n)
      dimension w(n)
!$acc data copyin(map,w) present(supg)
      val=0.0d0
!$acc parallel loop reduction(+:val) 
cc!$acc& gang vector vector_length(32)
      do i=1,n
         val=val+w(i)*supg(map(i))
      enddo
!$acc end data
      return
      end

      subroutine gatsubx(n,supg,map,w,mm,val)
      implicit REAL (a-h,o-z), integer (i-n)
INCLUDE(common/tractlt)
      dimension supg(0:n2int),map(n)
      dimension w(mm)
!$acc data copyin(map,w) present(supg)
      val=0.0d0
!$acc parallel loop reduction(+:val) gang vector
!$acc& vector_length(64)
      do i=1,mm
         val=val+w(i)*(supg(map(i))-supg(map(mm+i)))
      enddo
!$acc end data
      return
      end
...
c
      implicit REAL (a-h,o-z), integer (i-n)
INCLUDE(common/tractlt)
      dimension r(n),supg(0:n2int),map(n)
c
!$acc kernels present (supg) copyin(map) copyout (r)
cc!$acc loop gang vector(16)
      do 10 loop=1,n
...
c...  r = a - b
c
!$acc kernels copyin(a,b) copyout(r)
      do 10 i=1,n
         r(i)=a(i)-b(i)
10    continue
!$acc end kernels
c
      ...
      subroutine daxpyx(m,da,dx,incx,dy,incy)
      implicit REAL (a-h,o-z), integer (i-n)
      dimension dx(m),dy(m)
      if ((incx.ne.1).and.(incy.ne.1))stop 'daxpyx error'
      if (m.lt.1000) then
         do i=1,m
            dy(i)=dy(i)+da*dx(i)
         enddo
      else
!$acc kernels copyin(da,dx) copy(dy)
         do i=1,m
            dy(i)=dy(i)+da*dx(i)
         enddo
!$acc end kernels
