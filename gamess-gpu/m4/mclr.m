...
_IF(cray,ksr)
      call unpack (gin(409),16,ijij,408)
_ELSE
...
50    ibox = (ind1-1)/lp + 1
      ...
      if (btest(j,0)) then
_IF(cray,ksr)
         q(kpos+1) = shiftl(ind1,48).or.shiftl(ind2,32)
_ELSE
         itemp(1) = ind1
         itemp(2) = ind2
         q(kpos+1) = pack
...
_IF(cray,ksr)
         q(kpos) = q(kpos).or.(shiftl(ind1,16).or.ind2)
_ELSE
         pack = q(kpos)
         itemp(3) = ind1
         itemp(4) = ind2
...
c           iofhe = ia1 + ib1
     ...
     ?                      + npoint(ic)+nprm(isymc)
            ...
            do 390 ib=1,nbb
            a(iofab+ib) = a(iofab+ib) + focka * d(iofda+ib)
_IF1()c     if (odebug(34)) write (iwr,269) k,ix,ic,iofda+ib,focka,
_IF1()c     ?                                      d(iofda+ib),iofhe+ib 
_IF1()c269       format ('orbital pair ',i3,': adding fi(',i2,',',i2,') * d(',
_IF1()c     ?              i3,') = ',f9.4,' * ',f9.4,' to help(',i2,')')
...
_IF1()c         call mxmb  (d(iofda),1,naabb, w(ioffw),icol,1,
_IF1()c     ?       a(iofab),1,nci1, nbb,1,lenw)
_IF1()c         call mxmbn (d(iofdb),1,naabb, w(ioffw),icol,1,
_IF1()c     ?       b(iofab),1,nci1, nbb,1,lenw)
      end if
450   continue
...
690   continue
      kpos1 = kpos1 + 3 * np
700   continue
_IF1()c      if(odebug(78))then
_IF1()c  print*,'reordered integrals:'
_IF1()c  do n=1,np
_IF1()c  print*,'pair ',n
_IF1()c  call outvec (q(iws+(n-1)*listai),listai,' ')
_IF1()c  end do
_IF1()c      end if
...
_IF1()c  do n=1,np
_IF1()c  print*,'pair ',n
_IF1()c  call outvec (q(iws+(n-1)*listab),listab,' ')
_IF1()c  end do
_IF1()c      end if
c...output reordered integrals
      nw = nw + np*listab
...
INCLUDE(common/iofile)
INCLUDE(common/debug)
INCLUDE(common/lradrs)
INCLUDE(common/lrfile)
...
c   r = omega * s - e
c
_IF1(civu)150   call gtrian(npar2,etare(j)-cshift,e,e,s)
_IFN1(civu)150   call vsmsb(s,1,etare(j)-cshift,e,1,e,1,npar2)
c
...
c compute
            call lrmult (3,.false.,civec,d,x,s,e,q,q)   
c (omega * S - E) v~(j)
_IF1(civu)      call gtrian(npar2,etare(j),e,e,s)
_IFN1(civu)      call vsmsb(s,1,etare(j),e,1,e,1,npar2)
...
c---------------------------------------------------------------------
c
      implicit REAL (a-h,p-w), logical (o)
...
_IF1()      done = 1.0d0
_IF1()      do 20 isyma=1,nirr
_IF1()      call daxpy(mact(isyma)
_IF1()     +    ,-1.0d0,done,0,q(id1+iofd(isyma)) 
_IF1()     +    ,mact(isyma)+1)
_IF1()20    call daxpy(mact(isyma)
_IF1()     +    ,-2.0d0,done,0,q(id2+iofd(isyma)) 
_IF1()     +    ,mact(isyma)+1)
      do 20 isyma=1,nirr
      iyy1 = id1 + iofd(isyma)
      iyy2 = id2 + iofd(isyma)
...
_IF(cray,fps,cyber205,ksr)
      subroutine pack22 (nword,iii,i,j)
c---------------------------------------------------------------------
c   Packs four integer numbers into one word.
c   (c) Carsten Fuchs 1991
c---------------------------------------------------------------------
      integer  iii(*),i(*),j(*)
      nw = (nword-1)/2 + 1
      kk = 1
      do 10 k=1,nw
      iii(k) = shiftl(i(kk),48)  .or.shiftl(j(kk),32).or.
     +         shiftl(i(kk+1),16).or.       j(kk+1)
10    kk = kk + 2
      return
      end
_ELSE
