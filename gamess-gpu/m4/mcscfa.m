...
INCLUDE(common/runlab)
     ...
     1          ,'null    ','internal'/
c
_IF1(i)      call errset(187,256,-1,2,1,0)
c
...
10    continue
      ...
      ivecm = ivec - 1
      do 20 i=1,nci
20    q(ivecm+i)=( dfloat(i)+4.73265d0) / ( dfloat(nci)+3.87453d0)
...
_ELSE
      if ((nref+1)*nact .gt. 1356) call caserr (
_ENDIF
...
_IF(vax)
      ...
      mmax = 0
      do 40 j=1,nbasao
      if ( dabs(vec(j,i)).le.vmax) goto 40
      mmax = j
      vmax =  dabs(vec(j,i))
  40  continue
_ELSE
...
_IF(vax)
      ...
      mmax = 0
      do 10 j=1,nbasao
      if ( dabs(vec(j,i)).le.vmax) goto 10
      mmax = j
      vmax =  dabs(vec(j,i))
  10  continue
_ELSE
...
20    iq(iplis+iplist(i))=i
      ...
      npp=npp+1
_IF(bits8)
      call packft(iii,iii,ival(iw),iorb(iw),jorb(iw),0,0,
     +            out(npp))
_ELSE
...
40    continue
      ...
      npp=npp+1
_IF(bits8)
      call packft(iii,jjj,ival(iw),iorb(iw),jorb(iw),0,0,
     +            out(npp))
_ELSE
...
c      print31,npp,iii,jjj,ival(iw),iorb(iw),jorb(iw),0,0,out(npp)
      ...
      npp=mod(npp,511)+1
_IF(bits8)
      call packft(jjj,iii,ival(iw),jorb(iw),iorb(iw),0,0,
     +            out(npp))
_ELSE
...
c
      ...
      npp=npp+1
_IF(bits8)
      call packft(iii,iii,ival(iw),iorb(iw),jorb(iw),
     +            korb(iw),lorb(iw),out(npp))
_ELSE
...
60    continue
      ...
      npp=npp+1
_IF(bits8)
      call packft(iii,jjj,ival(iw),iorb(iw),jorb(iw),
     +            korb(iw),lorb(iw),out(npp))
_ELSE
...
c     >,out(npp)
      ...
      npp=mod(npp,511)+1
_IF(bits8)
      call packft(jjj,iii,ival(iw),jorb(iw),iorb(iw),
     +            lorb(iw),korb(iw),out(npp))
_ELSE
...
_IF(cray,ibm,vax)
      do 50 i=1,nci
   50 sigma(i,istate) = sigma(i,istate) + vec(i,istate)*q(idiag-1+i)
_ELSE
...
_EXTRACT(detpp,_AND(hp800,i8))
_IF(hpux11)
c HP compiler bug JAGae56132
...
_ELSEIF(convex,i8drct)
      integer *8 patern,patset,pat1,pat2,dif,bra,zket
_ELSE
      REAL  patern,patset,pat1,pat2,dif,bra,zket
_ENDIF
INCLUDE(common/sizes)
INCLUDE(common/multic)
      parameter (maxp=40,maxst=5,maxpst=maxp+maxst)
...
60    bra = iand(dif,pat1)
      zket = iand(dif,pat2)
_ELSEIF(bits8)
60    call dand8(dif,pat1,bra)
      call dand8(dif,pat2,zket)
_ELSE
60    bra = dand(dif,pat1)
      zket = dand(dif,pat2)
_ENDIF
      i11 = leadz(bra)+1
_IF(convex,i8)
...
_ELSEIF(bits8)
      call shiftl8(bra,i11,tmp8)
      i12 = leadz(tmp8)+1+i11
_ELSE
      i12 = leadz(shiftl(bra,i11))+1+i11
_ENDIF
      i21 = leadz(zket)+1
_IF(convex,i8)
      i22 = leadz(ishft(zket,i21))+1+i21
_ELSEIF(bits8)
      call shiftl8(zket,i21,tmp8)
      i22 = leadz(tmp8)+1+i21
_ELSE
      i22 = leadz(shiftl(zket,i21))+1+i21
_ENDIF
...
_ELSEIF(bits8)
      call shiftl8(dif,i1,tmp8)
      i2 = leadz(tmp8)+1+i1
      call shiftl8(dif,i2,tmp8)
      i3 = leadz(tmp8)+1+i2
      call shiftl8(dif,i3,tmp8)
      i4 = leadz(tmp8)+1+i3
      call shiftr8(pat1,65-i2,tmp1)
      call shiftl8(tmp1,65-i2+i1,tmp2)
      call shiftr8(pat1,65-i4,tmp3)
      call shiftl8(tmp3,65-i4+i3,tmp4)
      if (odd( popcnt(tmp2) +  popcnt(tmp4) ) ) val=-val
_ELSE
      i2=leadz(shiftl(dif,i1))+1+i1
      i3=leadz(shiftl(dif,i2))+1+i2
      i4=leadz(shiftl(dif,i3))+1+i3
      if( odd( popcnt(shiftl(shiftr(pat1,65-i2),65-i2+i1))
     1       + popcnt(shiftl(shiftr(pat1,65-i4),65-i4+i3)) ) )val=-val
_ENDIF
      val=val*2
      goto 220
c
...
_ELSEIF(bits8)
      call shiftl8(dif,i11,tmp8)
      i21 = leadz(tmp8)+1+i11
_ELSE
      i21=leadz(shiftl(dif,i11))+1+i11
_ENDIF
      if(odebug(34)) then
...
_ELSEIF(bits8)
      call shiftr8(pat1,65-i21,tmp8)
      call shiftl8(tmp8,65-i21+i11,tmp88)
      parity = odd(popcnt(tmp88))
_ELSE
      parity = odd(popcnt(shiftl(shiftr(pat1,65-i21),65-i21+i11)))
_ENDIF
      val = 0.0d0
...
c.... one electron
      do 50 iw=1,nwa
      iad = iwa(iw,1)+itzmtb
50    hrow(iad) = hrow(iad) +  dfloat(iwa(iw,2))*zint(nint2+iwa(iw,3))
      do 60 iw=1,nwb
      iad = iwb(iw,1)+icmta
60    hrow(iad) = hrow(iad) +  dfloat(iwb(iw,2))*zint(nint2+iwb(iw,3))
c      print*,'one electron done'
      end if
c...  two electron a->a b->b
      do 80 iw1=1,nwa
      par =  dfloat(iwa(iw1,2))
      io = intoff(isymr)+(iwa(iw1,3)-1)*npair(isymr)
c      print*,'iw1,io,par ',iw1,io,par
      do 70 iw2=1,nwb
      iad = iwa(iw1,1)+iwb(iw2,1)
c      print*,'iw2,iad ',iw2,iad
70    hrow(iad)=hrow(iad)+par*dfloat(iwb(iw2,2))*zint(io+iwb(iw2,3))
80    continue
90    continue
c
...
_ENDIF
_ENDEXTRACT
_EXTRACT(twoel,_AND(hp800,itanium))
...
c
      maxrp3 = maxrpl*3
      sz = ( dfloat(na-nb))*0.5d0
      fac =  dfloat(nb) + sz*(sz+1.0d0)
_IF(cray,ibm,vax)
       do 1 i=1,nci*nvec
    1 s(i) = fac*c1(i)
_ELSE
...
c..   symmetry adaption has been done - read it in
        ...
        nirrr=nirr
_IF1(c)        call fmove (isymao,symaos,maxorb)
_IFN1(c)        call icopy (maxorb,isymao,1,symaos,1)
      end if
...
_ELSEIF(convex,i8drct)
      integer *8 pat,patset,z1,z0
_ELSE
      REAL pat,patset,z1,z0
_ENDIF
_IF(i8drct)
      integer *8 shiftl, shiftr
      external shiftr,shiftl
_ENDIF
INCLUDE(common/sizes)
INCLUDE(common/multic)
...
_ELSEIF(bits8)
      call shiftl8(pat,j-jlast,pat)
_ELSE
      pat = shiftl(pat,j-jlast)
_ENDIF
      jlast=j
_IF(cray,t3e,convex,i8,i8drct)
      pat=ior(pat,z1)
_ELSEIF(bits8)
      call dor8(pat,z1,pat)
_ELSE
      pat=dor(pat,z1)
_ENDIF
    1 continue
...
_ENDIF
      jlast=j
_IF1() PAUL cray code was missing here?
_IF(convex,i8,cray,t3e)
    2 pat=ior(pat,z1)
      patset=ishft(pat,64-jlast)
_ELSEIF(bits8)
    2 call dor8(pat,z1,pat)
      call shiftl8(pat,64-jlast,patset)
_ELSEIF(i8drct)
    2 pat=ior(pat,z1)
      patset=shiftl(pat,64-jlast)
_ELSE
    2 pat=dor(pat,z1)
      patset=shiftl(pat,64-jlast)
_ENDIF
...
_IF1()  packft and unpkft should be compatible
_IF1()   unix machines follow vax (convex/alliant only for pack)
_IF(bits8)
      subroutine packft(ii,jj,ival,i,j,k,l,packed)
_ELSE
      function packft(ii,jj,ival,i,j,k,l)
_ENDIF
      implicit REAL  (a-h,o-z)
_IF(cray,t3d)
      ans = shiftl(ii,48).or.shiftl(jj,32).or.shiftl(ival,20)
     >.or.shiftl(i,15).or.shiftl(j,10).or.shiftl(k,5).or.l
_ELSEIF(ksr)
      integer packft,ans
      ans = or(shiftl(ii,48),or(shiftl(jj,32),or(shiftl(ival,20),
     >         or(shiftl(i,15),or(shiftl(j,10),or(shiftl(k,5),l)
     >            )))))
_ELSE
_IF(convex,i8drct)
      integer *8 packft,ans
_ENDIF
_IF(i8)
      integer *4 ians
      integer packft,ans
...
_IF(convex)
      integer *8 ig,ia
      integer *8 loop8
      common /lsort / ig(511),ib1(511),ib2(511),ib3(511),ib4(511)
_ELSEIF(cray,ksr,t3d)
      common /lsort / ig(511),ib1(511),ib2(511),ib3(511),ib4(511)
_ELSEIF(i8,i8drct)
      integer *4 ig
...
_IF(cray,ksr,t3d)
      ia = ig(i)
      ib1(i) = shiftr(ia,48)
      ia = ia - shiftl(ib1(i),48)
      ib2(i) = shiftr(ia,32)
      ia = ia - shiftl(ib2(i),32)
      ib3(i) = shiftr(ia,20)
      ia = ia - shiftl(ib3(i),20)
      ib4(i) = shiftr(ia,15)
      ia = ia - shiftl(ib4(i),15)
      ib5(i) = shiftr(ia,10)
      ia = ia - shiftl(ib5(i),10)
      ib6(i) = shiftr(ia,5)
      ib7(i) = ia - shiftl(ib6(i),5)
_ELSEIF(convex)
      ia = ig(i)
      loop8 = ishft (ia,-48)
      ib1(i) = loop8
      ia = ia - ishft (loop8,48)
      loop8 = ishft (ia,-32)
      ib2(i) = loop8
      ia = ia - ishft (loop8,32)
      ib3(i) = ishft (ia,-20)
      ia = ia - ishft (ib3(i),20)
      ib4(i) = ishft (ia,-15)
      ia = ia - ishft (ib4(i),15)
      ib5(i) = ishft (ia,-10)
      ia = ia - ishft (ib5(i),10)
      ib6(i) = ishft (ia,-5)
      ib7(i) = ia - ishft (ib6(i),5)
_ELSE
      ib1(i) = ig(1,i)/i16
...
_ENDIF
 10   continue
      ...
      implicit REAL  (a-h,o-z)
_IF(ibm,vax)
      logical*1 ij,kl,i1,j1,k1,l1
      common /lsort / gin(340),ij(680),kl(680),mword,ispc
      common/craypk/  i1(1360),j1(1360),k1(1360),l1(1360)
      ii=-1
      jj=0
      do 10 i=1,mword
      jj=jj+4
   ...
   10 l1(jj)=kl(ii+1)
      return
_ELSEIF(cray,ksr)
      common/lsort / gin(340),ij(170),mword,ispc
      common/craypk/  i1(340),j1(340),k1(340),l1(340)
     * ,pbuf(1360)
      call unpack(ij,8,pbuf,1360)
      call dcopy(mword,pbuf(1),4,i1,1)
      call dcopy(mword,pbuf(2),4,j1,1)
      call dcopy(mword,pbuf(3),4,k1,1)
      call dcopy(mword,pbuf(4),4,l1,1)
      return
_ELSEIF(i8)
...
_ELSE
      call icopy (mword,ipbuf(1),4,i1,1)
      call icopy (mword,ipbuf(2),4,j1,1)
      call icopy (mword,ipbuf(3),4,k1,1)
      call icopy (mword,ipbuf(4),4,l1,1)
_ENDIF
_ELSE
      common/lsort/gin(510),mword,ispc
      common/craypk/i1(340),j1(340),k1(340),l1(340)
...
_ELSEIF(convex)
      integer *8 ij
      common /scra  / gg(340),ij(170),nword,jcode
_ELSEIF(ibm,vax)
      common /scra  / gg(340),ij(340),m,mc
_ELSE
      integer*4 nword,jcode
      common /scra  / gg(510),nword,jcode
_ENDIF
      dimension gout(*),iout(*)
      call dcopy(mword,gout,1,gg,1)
c
_IF(cray)
      m=shiftl(mword,32).or.icode
      call pack(ij,32,iout,340)
_ELSEIF(ibm,vax)
      call fmove (iout,ij,(mword+1)/2)
      m=mword
      mc=icode
_ELSEIF(ksr)
      m = pack2(mword,icode)
...
_IF(fps)
      subroutine upkint
      implicit REAL  (a-h,o-z)
      word mij,mkl,mijii,mklii
      common /lsort / gin(340),mij(85),mkl(85),nwb
      common/craypk/  i1(340),j1(340),k1(340),l1(340)
      iw=0
      do 1 ii=1,85
      mijii=mij(ii)
      ...
      do 1 jj=1,4
      iw=iw+1
      i1(iw)=extract(mijii,iposi,8)
      j1(iw)=extract(mijii,iposj,8)
      k1(iw)=extract(mklii,iposi,8)
      l1(iw)=extract(mklii,iposj,8)
      if(nwb.eq.iw)go to 2
      iposi=iposi+16
      iposj=iposj+16
   1  continue
   2  return
...
c...  in 4-index
      ...
      common/lsort/g(510),mword,icode
      dimension gij(*),nij(*)
      itemp=extract(mword,0,32)
      icode=extract(mword,32,32)
      mword=itemp
      ii=1
      do 1 i=1,170
      nij(ii)=extract(gij(i),0,16)
      nij(ii+1)=extract(gij(i),16,16)
      nij(ii+2)=extract(gij(i),32,16)
      nij(ii+3)=extract(gij(i),48,16)
   1  ii=ii+4
      return
      end
      subroutine trout (gout,iout,mword,icode,num)
      ...
      dimension gout(*),iout(*)
      call dcopy(mword,gout,1,gg,1)
      m=insert(icode,0,32,mword)
      int=1
      do 1 i=1,170
      ij(i)=insert(iout(int+1),0,32,iout(int))
   1  int=int+2
c
      call put (gg,511,num)
      return
...
c...  in 4-index
      ...
      itemp=mword
      call upack2(itemp,mword,icode)
c..    call upak4v(gij,nij)      may be tried
       do 10 i=1,170
          nij(i*4-3) = and(shift(gij(i),16),mask)
          nij(i*4-2) = and(shift(gij(i),32),mask)
          nij(i*4-1) = and(shift(gij(i),48),mask)
          nij(i*4) = and(gij(i),mask)
10     continue
       return
      end
      subroutine trout (gout,iout,mword,icode,num)
      ...
      m = pack2(mword,icode)
       do 10 i=1,170
10     ij(i) = or(shift(iout(i*2-1),32),iout(i*2))
      call put (gg,511,num)
      return
      end
_ENDIF
