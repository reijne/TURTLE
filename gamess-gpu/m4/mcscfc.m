...
147   continue
      goto 146
_IF(bits8)
148   call packft(ii(iw),jj(iw),izero,iorb(iw),jorb(iw),
     +            korb(iw),lorb(iw),g(iw) )
_ELSE
...
_ELSEIF(convex,i8drct)
      integer *8 out1,out2,packft
_ELSEIF(bits8)
      REAL out1,out2
_ELSE
      REAL out1,out2,packft
_ENDIF
      integer orbtbf
INCLUDE(common/sizes)
      parameter (mxorb1=maxorb+1)
...
INCLUDE(common/multic)
     ...
     1 ,itrak,isegt,diagon,nw1,nw2,nform1,nform2
      ...
      ival=mcsurd(val)
_IF(bits8)
      call packft(ii,jj,ival,orbtbf(i),orbtbf(j),0,0,
     +            out1(nw1))
_ELSE
...
INCLUDE(common/multic)
     ...
     1 ,itrak,isegt,diagon,nw1,nw2,nform1,nform2
      ...
      ival=mcsurd(val)
_IF(bits8)
      call packft(ii,jj,ival,orbtbf(i),orbtbf(j),orbtbf(k),orbtbf(l),
     +            out2(nw2))
_ELSE
      out2(nw2)=
...
_IF(sgi,sun,ipsc,rs6000,hp700,hpux11,ksr)
      if(kseg.le.125) then
_ENDIF
...
_IF(sgi,sun,ipsc,rs6000,hp700,hpux11,ksr)
     e 1,24,24,11,11,81,11,13,1,27) kseg
       else
       kseg1 = kseg-125
       go to(27,1,28,1,3,2,4,2,1,2,2,1,29,63,64,
_ELSE
