_IFN(nbo)
      subroutine runnbo
      call caserr('NBO not available in this version')
      end
_ELSE
c
...
cold      parameter(maxatm = 99,maxbas = 500)
      parameter (maxatm = MAXAT, maxbas = MAXORB)
...
c
        ...
        if(nomac.ne.0) then
          pcent = sumac/dfloat(nomac) * 100.0d0
...
c
      if(jprint(5).eq.1.and.nel.ne.0) then
        write(lfnpr,2000)
        if(ipseud.ne.0) write(lfnpr,2010) dfloat(mecp)
        if(mcr.ne.0) then
...
c
        ...
        do 20 iat = 1,molat(imol)
          ...
          mecp = mecp + dfloat(kat - iznuc(molec(imol,iat)))
...
c
      ...
      REAL eiga(maxorb), frocca(maxorb),eigb(maxorb), froccb(maxorb)
      REAL zero, dum
...
c
      ...
      REAL en, ehf, ehfa, ehfb, ehf1, ehf2, edel
      REAL two
c
c functions
c
...
_IF(cray,convex,titan)
          call caserr('nss not set in nbo for cray')
          call hstar(a(ipt1),a(ipt3),a(ipt6),nopk,nss)
_ELSE
...
c     integer*1 string(160)
c     NOTE: integer*1 is NOT recognised by quite a few compilers e.g. SUN
      logical *1 string(160), iii(4)
      equivalence (iii(1),mark)
      data mark/0/
c...  convert to lower case (use ascii table)
c
      do 125 i=1,160,4
         iii(4) =string(i)
         if (mark.ge.65.and.mark.le.90) then
          mark = mark+32
          string(i) = iii(4)
         endif
125   continue
