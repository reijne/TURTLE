...
_EXTRACT(jkint_dft,opteron,xeon,em64t)
     ...
     &     imode,aaa,bbb,ccc,ddd, schwarz_ao, schwarz_cd)
      ...
      REAL gout, aaa, bbb, ccc, ddd, schwarz_ao, schwarz_cd
...
c 
      ...
      logical oskipp
      REAL q4, tijl, pi, schwarz_lim, test, schw_max

      REAL dt0, dt1, tim0, time, tim1
...
c  check use of basi
      ...
      l2 = nshell(basi)*(nshell(basi)+1)/2
      if(ntchnk.le.0)then
         write(iwr,*)'chunk factor not initialised',ipg_nnodes(), 
     &        ntchnk, limchnk
...
c
     ...
     &                             imc,aaa,nbasfn(basi))
_IF(ga)
                           else if(imode.eq.61)then
c
c 2 centre cases, just copy integrals into global array g_2c
c
                              call mat_form_2c_ga(gout,ii,kk,
     &                             basi, bask)
_ENDIF
                           else
...
c 3 centre fock build, cfit in aaa, fock in ccc
         ...
         if(imode .eq. 6)then 
            nlen =  nbasfn(basi)*nbasfn(bask)
            call pg_dgop(2001,aaa,nlen,'+')
         endif
...
_IF(parallel)
c  Functions
      integer iipsci, ipg_nnodes, lenwrd
      logical oipsci
_ENDIF
c
c Local variables
...
_IF(ga)
c
c 2 centre integrals have been stored by pre_fit
c
                              call caserr('invalid imode with GA-tools')
_ELSE
c
c 2 centre cases, just copy integrals into aaa indexed (i,k)
c
...
_IF(parallel)
     ...
     +       nint_nonzero
          ...
          do i = 1, ipg_nnodes() - 1
            ...
            call pg_rcv(100,ibuff,3*liw,ilen,i,ifrom,1)

            nshl_drct_t = ibuff(1)
            nshl_incore_t = ibuff(2)
            nint_nonzero_t = ibuff(3)

            write(iwr,12348) i,
     +      nshl_drct_t,
     +      100.0d0*dble(nshl_drct_t)/(nshl_drct_t+nshl_incore_t),
     +      nshl_incore_t,
     +      100.0d0*dble(nshl_incore_t)/(nshl_drct_t+nshl_incore_t),
     +      nint_nonzero_t
          ...
          ibuff(1) = nshl_drct
          ibuff(2) = nshl_incore
          ibuff(3) = nint_nonzero
          call pg_snd(100,ibuff,3*liw,0,1)
       endif
12348  format(1x,'Node ',i3,' Direct =',i10,' (',
     +         f5.1,'%) : InCore =',i10,
...
_IF(ga)
c
c 2 centre integral where put in the right place by pre_fit
c
                           call caserr('invalid imode with GA-tools')
_ELSE
...
INCLUDE(common/dft_shlnos)
      ...
      REAL cxyz, a, r, p1, q1, r1, dij
      integer ijd
      common/junkx/cxyz(3,5625),
     +     a(mxp2),r(mxp2),p1(mxp2),q1(mxp2),r1(mxp2),dij(16*mxp2),
     +     ijd(225)
c
      REAL sqrt3,  sqrt5, sqrt7, done
c
c Local
c
      REAL ai, arri, axi, ayi, azi, csi, cpi, cdi, cfi, cgi 
      REAL aj, csj, cpj, cdj, cfj, cgj 
      REAL aa, aa1, dum, dum1, dum2
...
INCLUDE(../m4/common/sizes)
      ...
      REAL pint, qint, rint, aa, r, p1, q1, r1, dd
      integer ijd
...
INCLUDE(common/dft_denss)
      ...
      REAL c10, u2, a, ab, aandb, expe, rho, dum
      REAL ak, al
      REAL pa, qa, ra, pb, qb, rb, d2, pc, qc, rc
      REAL akxk, akyk, akzk
      REAL axac, ayac, azac
      REAL bxbc, bybc, bzbc
      REAL bbrrk, b, b1, brrk
      REAL c3x, c4x, c3y, c4y, c3z, c4z, csl, csk

      integer ijn1, ijn2
      REAL pi252, dzero, pt5, done, factor
c
...
INCLUDE(common/dft_dmisc)
      ...
      REAL f1, rt1, g, ww1, f00, y, u2, ai, aj, ak, al
      REAL xa, ya, za, xb, yb, zb, xd, yd, zd
      REAL rho, xinv, x, a, b, b1
      REAL c1x, c1y, c1z, c2x, c2y, c2z
      REAL axad, ayad, azad
      REAL akxk, akyk, akzk
      REAL bxbd, bybd, bzbd
      REAL brrk, bbrrk, exkl, expe, csk
      REAL aandb, ab, alpha
      integer joff, n, nn, ig, jg, kg, lg, iper, i, jgmax

      REAL pt5, pt2, third, one, pi252, pie4
...
INCLUDE(common/dft_shlnos)
     ...
     +   ijd(225)
      REAL done
      REAL ai, arri, axi, ayi, azi
      REAL aj, aa, aa1, dum, csi
...
INCLUDE(common/dft_shlinf)
      ...
      REAL pin, qin, rin,aa,r,p1,q1,r1,dd
      integer ijd
...
c
      REAL axac, ayac, azac
      REAL axad, ayad, azad, bxbd, bybd, bzbd
      REAL bxbc, bybc, bzbc
      REAL csk, cpk, cdk, cfk, cgk
      REAL csl, cpl, cdl, cfl, cgl
      REAL c1x, c2x, c3x, c4x
      REAL c1y, c2y, c3y, c4y
      REAL c1z, c2z, c3z, c4z
      REAL brrk, bbrrk, akxk, akyk, akzk
      REAL dum, dum1, dum2, a, b, ab, aandb
      REAL ai, aj, ak, al
      REAL pb, qb, rb
      REAL pa, qa, ra
      REAL pc, qc, rc
      REAL pd, qd, rd
      REAL a1, b1, rho, expe, factor, u2
      logical odoub, onorm
      ...
      REAL sqrt3,  sqrt5, sqrt7, done, pi252, pt5
...
INCLUDE(common/dft_misc)
      ...
      REAL e, f1, pp, p, q, aa, ab, ww1, expe
      REAL csk, bxk, byk, bzk, bk, brrk
      REAL bbx, bby, bbz, sum, bbrrk, dum, bb, bl
      REAL bb1, d2, gout

      integer kg, lg, nn, n, lgmax

      REAL pi252, pie4, dzero, done
      data pi252 /34.986836655250d0/
      data pie4 /7.85398163397448d-01/
