...
typedef enum
{
  REG_NOERROR_RX = 0,           /* Success.  */
  REG_NOMATCH_RX,		/* Didn't find a match (for regexec_rx).  */

  /* POSIX regcomp_rx return error codes.  (In the order listed in the
     standard.)  */
  REG_BADPAT_RX,		/* Invalid pattern.  */
  REG_ECOLLATE_RX,		/* Not implemented.  */
  REG_ECTYPE_RX,		/* Invalid character class name.  */
  REG_EESCAPE_RX,		/* Trailing backslash.  */
  REG_ESUBREG_RX,		/* Invalid back reference.  */
  REG_EBRACK_RX,		/* Unmatched left bracket.  */
  REG_EPAREN_RX,		/* Parenthesis imbalance.  */ 
  REG_EBRACE_RX,		/* Unmatched \{.  */
  REG_BADBR_RX,		/* Invalid contents of \{\}.  */
  REG_ERANGE_RX,		/* Invalid range end.  */
  REG_ESPACE_RX,		/* Ran out of memory.  */
  REG_BADRPT_RX,		/* No preceding re for repetition op.  */

  /* Error codes we've added.  */
  REG_EEND_RX,		/* Premature end.  */
  REG_ESIZE_RX,		/* Compiled pattern bigger than 2^16 bytes.  */
  REG_ERPAREN_RX		/* Unmatched ) or \); not returned from regcomp_rx.  */
} reg_errcode_t;
...
#define REGS_FIXED 2
   ...
   * regexp.
   *
   * For POSIX compatability, when using regcomp_rx and regexec_rx this field
  ...
  unsigned LONG_INT allocated;	/* Size of that memory. */
...
//#ifndef _H_REGEX
typedef struct re_pattern_buffer regex_t_rx;
...
typedef struct
{
  regoff_t_rx rm_so;  /* Byte offset from string's start to substring's start.  */
  regoff_t_rx rm_eo;  /* Byte offset from string's start to substring's end.  */
} regmatch_t_rx;
//#endif
...
//#ifndef _H_REGEX
/* POSIX `cflags' bits (i.e., information for `regcomp_rx').  */

/* If this bit is set, then use extended regular expression syntax.
   If not set, then use basic regular expression syntax.  */
#define REG_EXTENDED_RX 1

/* If this bit is set, then ignore case when matching.
   If not set, then case is significant.  */
#define REG_ICASE_RX (REG_EXTENDED_RX << 1)
 
/* If this bit is set, then anchors do not match at newline
     characters in the string.
   If not set, then anchors do match at newlines.  */
#define REG_NEWLINE_RX (REG_ICASE_RX << 1)

/* If this bit is set, then report only success or fail in regexec_rx.
   If not set, then returns differ between not matching and errors.  */
#define REG_NOSUB_RX (REG_NEWLINE_RX << 1)


/* POSIX `eflags' bits (i.e., information for regexec_rx).  */
...
#define REG_NOTBOL_RX 1

/* Like REG_NOTBOL_RX, except for the end-of-line.  */
#define REG_NOTEOL_RX (1 << 1)
//#endif
...
RX_DECL unsigned LONG_INT rx_bitset_hash (INT size, rx_Bitset b);
...
RX_DECL unsigned LONG_INT rx_bitset_hash ();
...
extern INT regcomp_rx (regex_t_rx * preg, __const__ char * pattern, INT cflags);
extern INT regexec_rx (__const__ regex_t_rx *preg, __const__ char *string,
		    size_t nmatch, regmatch_t_rx pmatch[],
		    INT eflags);
extern size_t regerror_rx (INT errcode, __const__ regex_t_rx *preg,
			char *errbuf, size_t errbuf_size);
extern void regfree_rx (regex_t_rx *preg);
...
extern char * re_comp ();
extern INT re_exec ();
extern INT regcomp_rx ();
extern INT regexec_rx ();
extern size_t regerror_rx ();
extern void regfree_rx ();
...
struct rx_search_state
{
  ...
  regoff_t_rx * lparen;		/* scratch space for register returns */
  regoff_t_rx * rparen;
  regoff_t_rx * best_lpspace;	/* in case the user doesn't want these */
  regoff_t_rx * best_rpspace;	/* values, we still need space to store
...
#endif
{
  ...
  switch (pc)
	...
	{
	  search_state.best_lpspace =
	    (regoff_t_rx *)REGEX_ALLOCATE (search_state.num_regs * sizeof(regoff_t_rx));
	  search_state.best_rpspace =
	    (regoff_t_rx *)REGEX_ALLOCATE (search_state.num_regs * sizeof(regoff_t_rx));
	  ...
	  if (rxb->regs_allocated == REGS_UNALLOCATED)
	    { /* no.  so allocate them with malloc.  we need one
		 ...
		 gnu code uses.  */
	      ...
	      regs->start = ((regoff_t_rx *)
			     malloc (regs->num_regs * sizeof ( regoff_t_rx)));
	      regs->end = ((regoff_t_rx *)
			   malloc (regs->num_regs * sizeof ( regoff_t_rx)));
	      ...
	      if (regs->num_regs < search_state.num_regs + 1)
		{
		  ...
		  regs->start = ((regoff_t_rx *)
				 realloc (regs->start,
					  regs->num_regs * sizeof (regoff_t_rx)));
		  regs->end = ((regoff_t_rx *)
			       realloc (regs->end,
					regs->num_regs * sizeof ( regoff_t_rx)));
		  if (regs->start == 0 || regs->end == 0)
	  ...
	  if (regs->num_regs < search_state.num_regs + 1)
	    {
	      search_state.best_lpspace =
		((regoff_t_rx *)
		 REGEX_ALLOCATE (search_state.num_regs * sizeof(regoff_t_rx)));
	      search_state.best_rpspace =
		((regoff_t_rx *)
		 REGEX_ALLOCATE (search_state.num_regs * sizeof(regoff_t_rx)));
	      search_state.best_lparen = search_state.best_lpspace;
	...
	search_state.backtrack_frame_bytes =
	  (sizeof (struct rx_backtrack_frame)
	   + (rxb->match_regs_on_stack
	      ? sizeof (regoff_t_rx) * (search_state.num_regs + 1) * 2
	      : 0));
		  ...
		  if (effect < 0)
			...
			case re_se_pushback:
			  ...
			  search_state.super
				...
				- (LONG_INT)(((struct rx_superstate *)0)
					 ->transitions)));
			  goto top_of_cycle;
			  break;
			case re_se_push0:
	...
	if (regs && !rxb->no_sub)
	  {
	    ...
	    regoff_t_rx * s = regs->start;
	    regoff_t_rx * e = regs->end;
