...
toldiffdir=$benchmarkdir/../../utilities/toldiff
...
# Set list of jobs
job_list=`cat $benchmarkdir/jobs.list | grep -v "#"`
...
# Print header
echo 
echo "The results of the benchmark examples"
echo "====================================="
echo 

for job in $job_list
  do
  if ( test -e $benchmarkdir/LOG/${job}.log )
      then
      if ( test $oldval -eq 1 )
	  then
          #
	  # Using old vtab stuff
          # 
	  ../../utilities/validate $job $benchmarkdir/LOG/${job}.log || error=1
      else
          #
	  # New toldiff approach
          #
	  result=`$toldiff $benchmarkdir/REF/${job}.log $benchmarkdir/LOG/${job}.log`
	  status=$?
	  printf "%30s %8s\n" ${job} ${result}
	  if ( test \( ${status} -eq 1 \) )
	      then
		  ...
		  then
		  tkdiff ${benchmarkdir}/REF/${job}.log ${benchmarkdir}/LOG/${job}.log
	      elif ( test  ${text} -eq 1 )
		  then
		  $toldiff --output full ${benchmarkdir}/REF/${job}.log ${benchmarkdir}/LOG/${job}.log
	      fi
	  fi
      fi
  else
