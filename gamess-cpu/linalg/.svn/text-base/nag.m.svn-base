...
c     1st december 1971
c
      integer ifail, i, n, ip1, j, ia, ib, p01aaf
      double precision srname
      REAL  x, y, a(ia,n), b(ib,n), dl(n)
...
c     triangle of array a
  ...
  300 continue
      ifail = 0
      return
  320 ifail = p01aaf(ifail,1,srname)
      do 340 j=i,n
         b(j,j) = dl(j)
  340 continue
      return
...
c     1st august 1971
c
      integer j, im1, im2, n, ib, iz
      REAL  b(ib,n), dl(n), z(iz,im2)
      do 80 j=im1,im2
...
c
      ...
      REAL  atol, f, g, h, hh, a(ia,n), d(n), e(n), z(iz,n)
      do 40 i=1,n
         do 20 j=i,n
            z(j,i) = a(j,i)
   20    continue
...
c
      ...
      double precision srname
      REAL  tol, xxxx, a(ia,n), r(n), v(iv,n), e(n), x02adf, x02aaf
...
c
      ...
      double precision srname
      REAL  tol, xxxx, a(ia,n), b(ib,n), r(n), v(iv,n), dl(n), e(n),
     * x02adf, x02aaf
...
c
      ...
      double precision srname
      REAL  b, f, h, acheps, g, p, r, c, s, d(n), e(n), z(iz,n)
...
c     1st december 1971
c
      integer ifail, i, n, ia, id, k, l, j, p01aaf
      double precision srname
      REAL  y, d1, x, eps, a(ia,n), p(n)
      data srname /8h f03aff /
      do 10 i=1,n
         p(i) = 0.0
   10 continue
      do 15 j=1,n
   ...
   80    p(k) = l
         d1 = d1*a(k,k)
         if (x.lt.8.0*eps) go to 200
  100    if (dabs(d1).lt.1.0) go to 120
         d1 = d1*0.0625
         id = id + 4
         go to 100
  120    if (dabs(d1).ge.0.0625) go to 140
         d1 = d1*16.0
         id = id - 4
         go to 120
  ...
  180 continue
      ifail = 0
      return
  200 ifail = p01aaf(ifail,1,srname)
      return
      end
...
c     1st december 1971
c
      integer isave, ifail, ifail1, j, ir, i, n, l, iaa, ibb, id2,
     * ia, ix, ib, p01aaf
      double precision srname
      REAL  d0, d1, xmax, bbmax, d11, d2, eps, a(ia,n), aa(iaa,n),
     * p(n), b(ib,ir), x(ix,ir), bb(ibb,ir)
      data srname /8h f04ahf /
      isave = ifail
      ifail1 = 0
      do 40 j=1,ir
         do 20 i=1,n
  ...
  100 continue
      do 140 j=1,ir
         ...
         do 120 i=1,n
            ...
            call x03aaf(a(i,1), n*ia-i+1, x(1,j), (ir-j+1)*ix, n,
     *       ia, 1, -b(i,j), 0.0, d11, d2, .true., ifail1)
            bb(i,j) = -d11
  120    continue
         if (bbmax.gt.d1*xmax) d1 = bbmax/xmax
         if (bbmax.gt.2.0*eps*xmax) id2 = 1
  140 continue
      if ((d1.gt.0.5*d0) .and. (l.ne.1)) go to 160
      d0 = d1
      if (id2.eq.1) go to 60
      ifail = 0
      return
  160 ifail = p01aaf(isave,1,srname)
      return
      end
...
c     1st august 1971
c
      integer i, n, i1, k, ir, ia, ib
      REAL  x, a(ia,n), p(n), b(ib,ir)
...
c     ist. april 1973
c
      integer p01aaf, i, n, it, ia, ifail
      double precision srname
      REAL  xxxx, d1, a(ia,n), b(n), c(n), wks(n), x02aaf
      data srname /8h f04arf /
      do 20 i=1,n
         c(i) = b(i)
   20 continue
      it = 1
...
c     1st. april 1973
c
      integer p01aaf, i, n, j, it, iaa, ifail, ia
      double precision srname
      REAL  xxxx, d1, a(ia,n), b(n), c(n), aa(iaa,n), wks1(n),
     * wks2(n), x02aaf
      data srname /8h f04atf /
      do 40 i=1,n
         do 20 j=1,n
            aa(i,j) = a(i,j)
   20    continue
   40 continue
      it = 1
      call f03aff(n, x02aaf(xxxx), aa, iaa, d1, i, wks1, it)
      if (it.eq.0) go to 60
      ifail = p01aaf(ifail,1,srname)
      return
   60 it = 1
      call f04ahf(n, 1, a, ia, aa, iaa, wks1, b, n, x02aaf(xxxx),
     * c, n, wks2, n, i, it)
      if (it.eq.0) go to 80
...
c     for fps ap-164
c     x02aaf = 2.0**(-51.0)
      x02aaf = z'79c8000000000000'
      return
      end
      REAL  function x02acf(x)
c     nag copyright 1975
c     mark 4.5 release
      REAL  x
c     * rmax *
...
c     for fps ap-164
c     x02acf = 2.0**1023.0*(1.0-2.0**(-52))
      x02acf = z'ffefffffffffffff'
      return
      end
...
c     x02adf = 2.0**(-974.0)*(1.0+2.0**(-51))
      x02adf = z'0668000000000001'
      return
      end
      REAL  function x02agf(x)
...
c     .. scalar arguments ..
      REAL  x
c     ..
      x02agf = z'0048000000000001'
      return
      end
      subroutine x03aaf(a, isizea, b, isizeb, n, istepa, istepb,
     * c1, c2, d1, d2, sw, ifail)
c     mark 10 re-issue. nag copyright 1982
c     single precision base version
c     (for use in implementations without double precision)
c
...
c
c     for this single precision version, all additional (i.e.
c     double) precision computation is performed by the auxiliary
...
c
   80 call x03aay(a, isizea, b, isizeb, n, istepa, istepb, c1, c2,
     * d1, d2)
      return
      end
      subroutine x03aay(a, isizea, b, isizeb, n, istepa, istepb,
     * c1, c2, d1, d2)
c     mark 10 release. nag copyright 1982
c     version for fps ap-164, using apmath64 library routines for
c     double precision computations
c
c     performs double precision computation for x03aaf
c
c     .. scalar arguments ..
      REAL  c1, c2, d1, d2
...
c     .. local scalars ..
      integer i, is, it
c     .. local arrays ..
      REAL  sum(2), sumt(2), t(2)
c     ..
      call daddrr(c1, c2, sum)
      if (n.lt.1) go to 80
      is = 1
      it = 1
      do 60 i=1,n
         call dmulrr(a(is), b(it), t)
         call dadd(sum, t, sumt)
         sum(1) = sumt(1)
         sum(2) = sumt(2)
         is = is + istepa
         it = it + istepb
...
c
      integer p01aaf
      character *8 srname
      dimension a(ia,n), b(n), c(n), aa(iaa,n), wks1(n),
     * wks2(n)
      data srname /  'f04atf'/
      do 40 i=1,n
         do 20 j=1,n
...
c     of x02aaf, x02abf and x02acf as follows
c
      if (x02abf(x)*x02acf(x).ge.1.0) x02agf = x02abf(x)
      if (x02abf(x)*x02acf(x).lt.1.0)
     *                            x02agf = (1.0+x02aaf(x))/x02acf(x)
c
c     the correct value should be defined as a constant,
c     possibly in some binary, octal or hexadecimal representation,
c     and inserted into the assignment statement below.
...
c
      sum = c1 + c2
      if (n.lt.1) go to 60
      sum=sum+ddot(n,a,istepa,b,istepb)
c     it = 1
c     is = 1
c     do 40 i=1,n
c        sum = sum + a(is)*b(it)
...
c     additional precision computation
c
c  80 call x03aay(a, isizea, b, isizeb, n, istepa, istepb, c1, c2,
c    * d1, d2)
      return
      end
      subroutine x04aaf(i,nerr)
...
_IFN(alpha)
      ...
      ifail1 = 0
      do 20 i=1,n
         dumy = ddot(n,a(i,1),ia,a(i,1),ia)
         if (dumy.le.0.0d0) go to 200
         p(i) = 1.0d0/ dsqrt(dumy)
   20 continue
      d1 = 1.0d0
      id = 0
      do 180 k=1,n
         ...
         istart = k
         do 40 i=istart,n
         dumy = -a(i,k) + ddot(k1,a(i,1),ia,a(1,k),1)
            a(i,k) = -dumy
            dumy =  dabs(dumy*p(i))
            if (dumy.le.dumx) go to 40
            dumx = dumy
            l = i
   40    continue
         if (l.eq.k) go to 80
   ...
   80    p(k) = l
         d1 = d1*a(k,k)
         if (dumx.lt.8.0d0*eps) go to 200
  ...
  120    if ( dabs(d1).ge.0.0625d0) go to 140
         ...
         id = id - 4
         go to 120
  140    dumx = -1.0d0/a(k,k)
         if (k2.gt.n) go to 180
         do 160 j=k2,n
         dumy = -a(k,j) + ddot(k1,a(k,1),ia,a(1,j),1)
            a(k,j) = dumx*dumy
  160    continue
  180 continue
      ifail = 0
      return
  200 ifail = nagmsg(isave,1,zrname)
      return
      end
      subroutine f04ahf(n, ir, a, ia, aa, iaa, p, b, ib, eps, q,ix, bb,
     * ibb, l, ifail)
      implicit REAL  (a-h,p-w),integer    (i-n),logical    (o)
      implicit character *8 (z),character *1 (x)
      implicit character *4 (y)
      dimension    a(ia,n),
     * aa(iaa,n),p(n), b(ib,ir), q(ix,ir), bb(ibb,ir)
      data zrname /' f04ahf'/
      isave = ifail
      ifail1 = 0
      do 40 j=1,ir
  ...
  100 continue
      do 140 j=1,ir
         ...
         bbmax = 0.0d0
         do 120 i=1,n
            if ( dabs(q(i,j)).gt.qmax) qmax =  dabs(q(i,j))
            if ( dabs(bb(i,j)).gt.bbmax) bbmax =  dabs(bb(i,j))
c           call x03aaf(a(i,1), n*ia-i+1, q(1,j), (ir-j+1)*ix, n,ia, 1,
c    *       -b(i,j), 0.0d0, d11, d2, .true., ifail1)
            call x03aaz(a(i,1),  q(1,j), n,ia, 1,
     *       -b(i,j), 0.0d0, d11, d2)
            bb(i,j) = -d11
c           bb(i,j) = b(i,j) - ddot(n,a(i,1),ia,q(1,j),1)
  120    continue
         if (bbmax.gt.d1*qmax) d1 = bbmax/qmax
         if (bbmax.gt.2.0d0*eps*qmax) id2 = 1
  140 continue
      ...
      ifail = 0
      return
  160 ifail = nagmsg(isave,1,zrname)
      return
      end
      subroutine f04ajf(n, ir, a, ia, p, b, ib)
      implicit REAL  (a-h,p-w),integer    (i-n),logical    (o)
   ...
   40 continue
      do 100 k=1,ir
         do 60 i=1,n
            i1 = i - 1
            d1 = b(i,k) + ddot(i1,a(i,1),ia,b(1,k),1)
            b(i,k) = -d1/a(i,i)
   60    continue
         b(n,k) = -b(n,k)
         if (n.eq.1) go to 100
         do 80 ii=2,n
            i = n - ii + 1
            i1 = i + 1
            d1 = b(i,k) + ddot(n-i,a(i,i1),ia,b(i1,k),1)
            b(i,k) = -d1
   80    continue
  100 continue
      return
   ...
   40 continue
      it = 1
      call f03aff(n, x02aaf(1.0d0), aa, iaa, d1, i, wks1, it)
      if (it.eq.0) go to 60
      ifail = nagmsg(ifail,1,zrname)
      return
   60 it = 1
      call f04ahf(n, 1, a, ia, aa, iaa, wks1, b, n, x02aaf(1.0d0),c, n,
     * wks2, n, i, it)
      if (it.eq.0) go to 80
      ifail = nagmsg(ifail,2,zrname)
      return
   80 ifail = 0
      return
      end
...
99999 format (1h0, 38herror detected by nag library routine , a8,
     * 11h - ifail = , i5//)
      ...
      implicit character *8 (z)
      implicit character *4 (y)
cibm  data tol/z3410000000000000/
      data tol /0.278d-16/
      x02aaf = tol
      return
      end
      ...
      implicit character *8(z)
      implicit character *4 (y)
cibm  data tol/z7fffffffffffffff/
      data tol/0.17d+39/
      x02acf = tol
      return
      end
      ...
      implicit character *8 (z)
      implicit character *4 (y)
cibm  data tol/z0d10000000000000/
      data tol/0.211758d-21/
      x02adf = tol
      return
      end
...
INCLUDE(../m4/common/iofile)
      ...
      subroutine f02amf(n, acheps, d, e, q, iz, ifail)
      implicit REAL  (a-h,p-w),integer    (i-n),logical    (o)
      implicit character *8 (z),character *1 (x)
      implicit character *4 (y)
      dimension d(n), e(n), q(iz,n)
      data zrname /' f02amf'/
      isave = ifail
      if (n.eq.1) go to 40
      do 20 i=2,n
  ...
  380 continue
      ifail = 0
      return
  400 ifail = nagmsg(isave,1,zrname)
      return
      end
      subroutine f01ajf(n, atol, a, ia, d, e, q, iz)
      implicit REAL  (a-h,p-w),integer    (i-n),logical    (o)
      implicit character *8 (z),character *1 (x)
      implicit character *4 (y)
      dimension a(ia,n), d(n), e(n), q(iz,n)
      do 40 i=1,n
         do 20 j=1,i
            q(i,j) = a(i,j)
   20    continue
  ...
  400 continue
      ...
      tol = x02aaf(1.0d0)
      call f02amf(n, tol, r, e, v, iv, ifail)
      if (ifail.ne.0) ifail = nagmsg(isave,ifail,zrname)
      return
      end
_ENDIF
_ENDIF
...
c      x02aaf = 2.0d0**(-50)
_IF1(i)      data t02aaf /z3410000000000000/
_IF1(t)      data  t02aaf /   '3CA0000000000001'x/
_IF1(x)      data t02aaf /  '3cc0000000000001'x/
_IFN1(ctix)      data t02aaf /0.278d-16/
_IF1(c)      t02aaf = smach(1)
...
_IF1(i)      data  t02acf /z7fffffffffffffff/
_IF1(t)      data  t02acf /   '7FEFFFFFFFFFFFFF'x/
_IFN1(ctix)      data t02acf/0.17d+39/
_IF1(x)      data t02acf /  '7fffffffffffffff'x/
_IF1(c)      t02acf = smach(3)
      x02acf = t02acf
      return
      end
_IF(cray,convex)
      function x02ajf(dumx)
c     * tol *
c     returns the ratio of the smallest positive real floating-
c     point number representable on the computer to eps
c
      implicit REAL  (a-h,o-z)
_IF1(x)      data t02adf /   '35ffffffffffffe'x/
_IF1(c)      t02ajf = smach(2)
      x02ajf = t02ajf
      return
      end
      function x02abf(dumx)
c     * tol *
c     returns the ratio of the smallest positive real floating-
c     point number representable on the computer to eps
c
      implicit REAL  (a-h,o-z)
_IF1(c)      data t02abf /   x'2004800000000004'/
      x02abf = t02abf
      return
      end
_ELSE
...
_IF1(i)      data  t02adf /z0d10000000000000/
_IF1(t)      data  t02adf /    '360000000000139'x/
_IFN1(it)      data t02adf/0.211758d-21/
      x02adf = t02adf
      return
      end
_ENDIF
...
_IF(cray,convex)
         if (s.lt.x02ajf(0.0d0)) s = norm/dfloat(nhs)
_ELSE
         if (s.lt.x02adf(0.0d0)) s = norm/dfloat(nhs)
_ENDIF
...
_IF(ibm)
            call x03aaf(a(i,1), n*ia-i+1, x(1,j), (ir-j+1)*ix, n,
     *       ia, 1, -b(i,j), 0.0d0, d11, d2, .true., ifail1)
            bb(i,j) = -d11
_ELSE
...
_IF(convex)
      data t02agf /'20000000000001'x/
_ELSEIF(ibm)
      data t02agf /z0210000000000001/
_ELSEIF(alliant,ipsc,titan,ieee)
      data t02agf /'1000000000007C'x/
_ELSEIF(apollo,sgi,rs6000,dec,sun,hp700,hpux11,ibm,ksr,linux)
      data t02agf /3.d-308/
_ELSEIF(cray)
      if (x02abf(x)*x02acf(x).ge.1.0) t02agf = x02abf(x)
      if (x02abf(x)*x02acf(x).lt.1.0)
     *      t02agf = (1.0+x02aaf(x))/x02acf(x)
_ELSE
c     there is a problem - no machine constant for this machine..
c     use the ieee value
      data t02agf /3.d-308/
_ENDIF
      x02agf = t02agf
      return
