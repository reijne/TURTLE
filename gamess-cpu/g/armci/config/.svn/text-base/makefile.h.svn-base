#$Id: makefile.h,v 1.5 2007-10-30 10:36:36 jmht Exp $
           FC = f77
           CC = cc
           AR = ar
           AS = as
...
ifneq (,$(findstring mpif,$(_FC)))
         _FC = $(shell $(FC) -v 2>&1 | awk ' /g77 version/ { print "g77"; exit }; /gcc version 4/ { print "gfortran"; exit }; /gcc version/ { print "g77"; exit };/pgf/ { apgfcount++}; END {if(apgfcount)print "pgf77"} ; / frt / { print "frt" ; exit }' )
endif
ifneq (,$(findstring mpicc,$(_CC)))
...
#-------------------------------------
ifeq  ($(_CPU),x86_64)
  _FC = $(shell $(FC) -v 2>&1 | awk ' /g77 version/ { print "g77"; exit }; /gcc version 4/ { print "gfortran"; exit }; /gcc version/ { print "g77"; exit }; /Path/ { print "pathf90" ; exit }; /efc/ { print "efc" ; exit }; /pgf90/ { pgf90count++}; /pgf77/ { pgf77count++}; END {if(pgf77count)print "pgf77" ; if(pgf90count)print "pgf90"} ')
  ifeq ($(_FC),pgf90)
     FOPT_REN= -fast -Mdalign -tp k8-64 -O3
  endif
  _FC = $(shell $(FC) -v 2>&1 | awk ' /g77 version/ { print "g77"; exit }; /gcc version 4/ { print "gfortran"; exit }; /gcc version/ { print "g77"; exit }; /Path/ { print "pathf90" ; exit }; /efc/ { print "efc" ; exit }; /ifc/ { print "ifort" ; exit };/ifort/ { print "ifort" ; exit }; /pgf90/ { apgf90count++}; /pgf77/ { apgf77count++}; END {if(apgf77count)print "pgf77" ; if(apgf90count)print "pgf90"} ')
  ifeq ($(_FC),g77)
