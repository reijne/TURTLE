...
c
     ...
     5        ifd2(nirred),ifd3(nirred),ife1(nirred)
      ...
      real*8 buf(lnbkt),t2(ndimw),tau(ndimw),t1o(ndimt1),t1n(ndimt1)
      real*8 e(nbf),w1(nsqvv),w2(nsqvv),cc(maxdim),bb(maxdim+1,maxdim+2)
      real*8 bb2(maxdim+1,maxdim+2)
      real*8 a0,ap5,a1,conv,xeo,xe,a2,escf,eccsd,eccsd2,rms
      real*8 test
      real*8 anmt1,anmt2
c
c
c +++ common for ccsd/ccsd(t) file usage
c
      integer in, iw, itap57, itap60, itap61, itap62, itap63
      integer itap64, itap65, itap69, itap90, itap98, itap99
      common /t_files/ in,iw,itap57,itap60,itap61,itap62,itap63,
     +                 itap64,itap65,itap69,itap90,itap98,itap99
c
c
      integer nirred, nbf, no, nv, ntriv, ntrio, nsqvv, nsqoo
      integer nsqov, lnbkt, nlist, nbka, nbkb, nbkc, nbkd1, nbkd2
      integer nbkd3, nbke1, nbke2, nbkf, ndimt1, ndimt2, norbs, ndimw
      common /t_parm/ nirred,nbf,no,nv,ntriv,ntrio,nsqvv,nsqoo,nsqov,
     +                lnbkt,nlist,nbka,nbkb,nbkc,nbkd1,nbkd2,nbkd3,
     +                nbke1,nbke2,nbkf,ndimt1,ndimt2,norbs,ndimw
c
c
      integer nit, maxit, maxdim, convi
      common /t_iter/ nit,maxit,maxdim,convi
c
      integer ntypr
      parameter (ntypr=40)
...
c
      ...
      implicit integer (a-z)
      real*8 cc(*)
      real*8 enuc,escf,eccsd
      integer ic(*)
      logical grad
      character*8 option
c
      real*8 a0, a1, a2, a3, a4, a5, a6, a9, ap5
...
c
 ...
 7001 format('   integer arrays take up ',i10,'  words or ',i10,
     1          '  real words ')
      ...
      real*8 t2(ndimw),buf(lnbkt),eccsd,escf,etot,a0
c
c
c +++ common for ccsd/ccsd(t) file usage
c
...
c
   ...
   93 continue
      ...
      integer iparm
      character*(*) label
      real*8 tcp0,tsy0,tel0,tcp1,tsy1,tel1
      save tcp0,tsy0,tel0
      if (iparm.eq.0)then
 ...
 99     format(/
     ...
     1  iooovt(nirred),ntr(nirred,2),isqov(nsqov)
      real*8 t2(ndimw),t1o(ndimt1),t1n(ndimt1),w1(ndimt1)
      real*8 a1
c
c
      integer nirred, nbf, no, nv, ntriv, ntrio, nsqvv, nsqoo
...
convexreal*16 det
      real*8 det
      real*8 t2(ndimt2),tau(ndimt2),t1o(ndimt1),t1n(ndimt1),cc(maxdim)
      real*8 bb(maxdim+1,maxdim+2),bb2(maxdim+1,maxdim+2)
      real*8 a0,a1,xm,xmax,xen,xfac,xadd,rms,sum1,sum2
c
c
c +++ common for ccsd/ccsd(t) file usage
c
...
11    continue
     ...
     1  ioff(nbf),itriv(ntriv),isqoo(nsqoo),isqov(nsqov)
      real*8 t2(ndimw),tau(ndimw),t1o(ndimt1)
      real*8 t11,t12,fac1,fac2,a1,a2,a0
c
c
      integer nirred, nbf, no, nv, ntriv, ntrio, nsqvv, nsqoo
      integer nsqov, lnbkt, nlist, nbka, nbkb, nbkc, nbkd1, nbkd2
...
11    continue
     ...
     1  itriv(ntriv),isqoo(nsqoo)
      real*8 t2(ndimw),tau(ndimw),t1(ndimt1)
      real*8 fac,a0,a1,ap5,t11
c
c
      integer nirred, nbf, no, nv, ntriv, ntrio, nsqvv, nsqoo
      integer nsqov, lnbkt, nlist, nbka, nbkb, nbkc, nbkd1, nbkd2
...
11    continue
     ...
     2  ivvoot(nirred),ivvoo(nirred,nirred)
      real*8 t2(ndimw),tau(ndimw),t1o(ndimt1)
      real*8 a0,a1,a2,fac1,t11,fac2,t12
c
c
      integer nirred, nbf, no, nv, ntriv, ntrio, nsqvv, nsqoo
      integer nsqov, lnbkt, nlist, nbka, nbkb, nbkc, nbkd1, nbkd2
...
c
   ...
   10 continue
     ...
     &        ifd3(nirred)
      real*8 t1(ndimt1),t2(ndimw),dints(ndimd),eints(ndime),
     &       fints(ndimf),buf1(mxv3),buf2(mxv3),
     &       eta(ndimt1),zeta(ndimd),epo(no),epv(nv),gam(ndimw),
     &       aaa(ndimf),ccc(ndime),ee(nbf)
      real*8 tx(lntx)
      real*8 d1,d2,d3,d4,d5,fac3
      real*8 emp4t,eccsd
      real*8 x1,x2,x3,y1,y2,y3,x11,y11,e1
c
c
      real*8 a0, a1, a2, a3, a4, a5, a6, a9, ap5
      common /t_adata/ a0,a1,a2,a3,a4,a5,a6,a9,ap5
...
c
      ...
      bcsym=ieor(ijksym-1,asym-1)+1
      do 912 bsym=1,nirred
      csym=ieor(bcsym-1,bsym-1)+1
      acsym=ieor(asym-1,csym-1)+1
...
c      do 102 j = 1,no
      ...
      ksym=ieor(ijksym-1,ijsym-1)+1
      if (ksym.gt.jsym)goto 102
      iksym=ieor(isym-1,ksym-1)+1
      jksym=ieor(jsym-1,ksym-1)+1
      jj=j-flov(jsym,1)+1
      d2 = d1 + ee(j)
...
c - form intermediate for zeta and eta
      ...
      absym=ieor(csym-1,ijksym-1)+1
      do 342 c=flov(csym,3)-no,flov(csym,4)-no
      cc=c-flov(csym,3)+no
      d4=d3-ee(c+no)
      do 344 bsym=1,csym
      asym=ieor(absym-1,bsym-1)+1
      if (asym.gt.bsym)goto 344
      acsym=ieor(asym-1,csym-1)+1
      bcsym=ieor(bsym-1,csym-1)+1
      limb=flov(bsym,4)-no
...
c (ia:kc)*t1(jb)
      ...
      asym=ieor(iksym-1,csym-1)+1
      absym=ieor(ijksym-1,csym-1)+1
      lnab=npr(absym,2,2)
      lnac=npr(iksym,2,2)
      lna=flov(asym,4)-flov(asym,3)+1
...
c
      do 50 asym = 1,nirred
      isym = ieor(busym-1,asym-1) + 1
      uisym = ieor(usym-1,isym-1) + 1
      fi = flov(isym,1)
      li = flov(isym,2)
      fa = flov(asym,3) - no
      la = flov(asym,4) - no
...
c
     ...
     2  ioff(nbf),itriv(ntriv),ifd2(nirred)
      real*8 tau(ndimw),buf(lnbkt),t2(ndimw)
c
c
c +++ common for ccsd/ccsd(t) file usage
c
...
c
     ...
     &        iovvv(nirred,nirred),novvvt(nirred),isqvv(nsqvv)
      real*8 eta(ndimt1),zeta(ndimd),epo(no),epv(nv),gam(ndimw),
...
c
      ...
      fsym=ieor(aisym-1,bsym-1)+1
      if (fsym.gt.bsym)goto 119
      fisym=ieor(fsym-1,isym-1)+1
      bisym=ieor(bsym-1,isym-1)+1
...
c 1234 continue
     ...
     &        jdx1(nirred),jdx2(nirred,nirred),itrio(ntrio),
     &        ifd3(nirred)
      real*8 t1(ndimt1),t2(ndimt2),dints(ndimd),eints(ndime),
     &       fints(ndimf),buf1(mxv3),buf2(mxv3),ee(nbf)
      real*8 a0,a1,a2,a3,a4,a5,a6,a9,ap5
      real*8 d1,d2,d3,d4,d5,fac
      real*8 emp4t,eccsd
      real*8 x1,x2,y1,y2,e1
      real*8 e11
c
c
c +++ common for ccsd/ccsd(t) file usage
...
c
      real*8 scr
      integer lscr, t1amp, t2amp, rint, fmat, emat, taumat
      integer z1amp, z2amp, amed, bmed
      parameter (lscr=100)
      common /t_int/ scr(lscr),t1amp,t2amp,rint,fmat,emat,taumat,z1amp,
     &               z2amp,amed,bmed
c
     ...
     1  ioff(nbf),itriv(ntriv),isqov(nsqov),fpbkf(nbkf),isqvv(nsqvv)
      real*8 t2(ndimw),buf(lnbkt),t1o(ndimt1)
c
c
c +++ common for ccsd/ccsd(t) file usage
c
...
c
      do 50 asym = 1,nirred
      bsym = ieor(bisym-1,asym-1) + 1
      bvsym = ieor(besym-1,bsym-1) + 1
      off2 = ifc(bvsym)
      off3 = npr(bvsym,3,2)
      fb = flov(bsym,3) - no
      lb = flov(bsym,4) - no
...
c
     ...
     1  isqov(nsqov),fpbkf(nbkf),isqvv(nsqvv),isqoo(nsqoo)
      real*8 t2(ndimw),buf(lnbkt),t1o(ndimt1),t1n(ndimt1),w1(nsqvv)
c
c
c +++ common for ccsd/ccsd(t) file usage
c
...
11    continue
     ...
     &        ifd4(nirred),ife1(nirred)
      real*8 bfin(mxv3),eints(ndime),fints(mxv3),t2(ndimw)
c
c
      integer nirred, nbf, no, nv, ntriv, ntrio, nsqvv, nsqoo
      integer nsqov, lnbkt, nlist, nbka, nbkb, nbkc, nbkd1, nbkd2
...
c
      jksym=ieor(jsym-1,ksym-1)+1
      iksym=ieor(isym-1,ksym-1)+1
      ijksym=ieor(isym-1,jksym-1)+1
      do 110 fsym=1,nirred
      nvf=flov(fsym,4)-flov(fsym,3)+1
      nof=flov(fsym,2)-flov(fsym,1)+1
      ifsym=ieor(isym-1,fsym-1)+1
      csym=ieor(jksym-1,fsym-1)+1
      lnab=npr(ifsym,2,2)
      lncf=npr(jksym,2,2)
      nvc=flov(csym,4)-flov(csym,3)+1
     ...
     +          ,nvf,a1,bfin(ad3),lnab)
      ...
      jfsym=ieor(jsym-1,fsym-1)+1
      csym=ieor(iksym-1,fsym-1)+1
      lnab=npr(jfsym,2,2)
      lncf=npr(iksym,3,2)
      nvc=flov(csym,4)-flov(csym,3)+1
  ...
  110 continue
     ...
     &        ifd4(nirred),ife1(nirred)
      real*8 bfin(mxv3),eints(ndime),fints(mxv3),t2(ndimw),ccc(ndime),
     &       aaa(mxv3),gam(ndimw)
c
c
      integer nirred, nbf, no, nv, ntriv, ntrio, nsqvv, nsqoo
...
c
      ...
      ksym=ieor(bisym-1,jsym-1)+1
      bksym=ieor(besym-1,ksym-1)+1
...
c
     ...
     &        novvvt(nirred),ifd4(nirred)
      real*8 t1(ndimt1),t2(ndimw),tmp2(ndimw)
      real*8 a2
c
c
c +++ common for ccsd/ccsd(t) file usage
...
c
     ...
     +                nbke1,nbke2,nbkf,ndimt1,ndimt2,norbs,ndimw
c
      real*8 diff,intget,t1get,t2get,tst1,wget,vget
      real*8 d1,d2,d3,d4,d5,fac
c
      real*8 scr
      integer lscr, t1amp, t2amp, rint, fmat, emat, taumat
...
c
   ...
   10 continue
     ...
     1                 isqoo,isqvv,isqov,flov,itriv,ifd2,ifd4,fpbkd1)
      ...
      real*8 buf1(lnbkt),buf2(ndimw),w1(ndimt1)
      real*8 a0
c
c
c +++ common for ccsd/ccsd(t) file usage
...
c
      ...
      gasym = ieor(visym-1,usym-1) + 1
      fga = flov(gasym,3) - no
      lga = flov(gasym,4) - no
      nga = lga - fga + 1
      do 40 vsym = 1,nirred
      isym = ieor(visym-1,vsym-1) + 1
      uvsym = ieor(usym-1,vsym-1) + 1
      fv = flov(vsym,1)
      lv = flov(vsym,2)
      fi = flov(isym,1)
...
c
      ...
      jsym = ieor(vsym-1,uisym-1) + 1
      fj = flov(jsym,1)
      lj = flov(jsym,2)
      do 30 usym = 1,nirred
      isym = ieor(uisym-1,usym-1) + 1
      uvsym = ieor(usym-1,vsym-1) + 1
      fu = flov(usym,1)
      lu = flov(usym,2)
      fi = flov(isym,1)
...
c
      ...
      besym = ieor(gasym-1,uvsym-1) + 1
      fbe = flov(besym,3) - no
      lbe = flov(besym,4) - no
      do 30 usym = 1,nirred
      vsym = ieor(uvsym-1,usym-1) + 1
      gavsym = ieor(gasym-1,vsym-1) + 1
      fu = flov(usym,1)
      lu = flov(usym,2)
      fv = flov(vsym,1)
      lv = flov(vsym,2)
...
c
   ...
   10 continue
     ...
     1  isqoo(nsqoo),ifd2(nirred),iff2(nirred),ife2(nirred),isqov(nsqov)
      ...
      real*8 t2(ndimt2),e(nbf),t3(nsqoo*no),t1(ndimt1)
      real*8 eints(ndime),fints(ndimf),dints(ndimw)
      real*8 a0,a1,a2,a3,a4,a5,a6,a9,e1,emp4t,eccsd,emp4x
      real*8 d1,d2,d3,d4,d5,fac,e2,e3,e4,e5,e6
      real*8 fia,fib,fic,fja,fjb,fjc,fka,fkb,fkc
      real*8 t1ia,t1ib,t1ic,t1ja,t1jb,t1jc,t1ka,t1kb,t1kc
      real*8 e11,e22,e33,e44,e55,e66
c
c
c +++ common for ccsd/ccsd(t) file usage
c
...
c
      ...
      do 40 ksym = 1,nirred
      ijsym = ieor(ksym-1,abcsm-1) + 1
      fk = flov(ksym,1)
      lk = flov(ksym,2)
      cksym = ieor(csym-1,ksym-1) + 1
c
      f3sym = ieor(bcsym-1,ksym-1) + 1
      f4sym = ieor(acsym-1,ksym-1) + 1
      ff3 = flov(f3sym,3) - no
      lf3 = flov(f3sym,4) - no
      fm1 = flov(f3sym,1) 
      lm1 = flov(f3sym,2)
...
c
      do 50 jsym = 1,nirred
      isym = ieor(jsym-1,ijsym-1) + 1
      fj = flov(jsym,1)
      lj = flov(jsym,2)
      fi = flov(isym,1)
      li = flov(isym,2)
      bjsym = ieor(bsym-1,jsym-1) + 1
      aisym = ieor(asym-1,isym-1) + 1
      iksym = ieor(isym-1,ksym-1) + 1
      jksym = ieor(jsym-1,ksym-1) + 1
c
      f1sym = ieor(absym-1,isym-1) + 1
      f2sym = ieor(absym-1,jsym-1) + 1
      f5sym = ieor(bcsym-1,jsym-1) + 1
      f6sym = ieor(acsym-1,isym-1) + 1
      ff1 = flov(f1sym,3) - no
      lf1 = flov(f1sym,4) - no
      fm3 = flov(f1sym,1) 
      lm3 = flov(f1sym,2)
...
c
      ...
      return
       ...
       real*8 tcpu,tsys,telp
       real*8 tstrt,tx,delt
