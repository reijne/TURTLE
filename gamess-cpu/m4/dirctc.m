...
c
      ...
      save ncall
_IF(parallel)
      call caserr2('parallel MRMP not implemented yet')
_ENDIF
c
      ncall = ncall + 1
c
...
c     call vclr(dmsk,1,3)
      do 20 i=1,nw
         ...
         do 10 j=1,nref
_IF(cray,ksr)
10       dmsk(i) = and(dmsk(i),conf(i,j))
20    dmsk(i) = or(dmsk(i),shiftr(dmsk(i),1))
_ELSEIF(convex,i8)
10       dmsk(i) = iand(dmsk(i),conf(i,j))
20    dmsk(i) = ior(dmsk(i),ishft(dmsk(i),-1))
_ELSEIF(i8drct)
10       dmsk(i) = iand(dmsk(i),conf(i,j))
20    dmsk(i) = ior(dmsk(i),shiftr(dmsk(i),1))
_ELSEIF(bits8)
10       call dand8(dmsk(i),conf(i,j),dmsk(i))
         call shiftr8(dmsk(i),1,tmp8)
20    call dor8(dmsk(i),tmp8,dmsk(i))
_ELSE
10       dmsk(i) = dand(dmsk(i),conf(i,j))
20    dmsk(i) = dor(dmsk(i),shiftr(dmsk(i),1))
_ENDIF
      call smpmsk(dmsk)
      end
...
_ELSEIF(convex,i8drct)
      integer *8 conf,resul,dmsk,rmsk
_ELSE
      REAL conf,resul,rmsk,dmsk
_ENDIF
...
c
      dimension iocc(62)
_IF()
chvd  creating the mask has been moved to separate subroutine
c     see subroutine mpmask
c      call vclr(dmsk,1,3)
c
...
c         dmsk(i) = n32m
c         do 10 j=1,nref
c10       dmsk(i) = dand(dmsk(i),conf(i,j))
c20    dmsk(i) = dor(dmsk(i),shiftr(dmsk(i),1))
_ENDIF
c
...
c
_IF(cray)
      call dcopy(10*18,ncmph2,1,ncmphd,1)
_ELSE
...
_ELSEIF(convex,i8drct)
      integer *8 conf,dmsk,resul
_ELSE
      REAL conf,dmsk,resul
_ENDIF
...
_ELSEIF(convex,i8drct)
      integer *8 conf,resul,dmsk,holem,temp
_ELSE
      REAL conf,resul,dmsk,holem,temp,dand
      logical eq
_ENDIF
c
      dimension conf(5,nconf),resul(5,*),dmsk(3)
...
c
      do 10 i=1,nconf
         do 5 j=1,nwm1
_IF(cray,ksr)
           temp(j) = and(dmsk(j),conf(j,i))
           if (temp(j).ne.holem(j)) go to 10
_ELSEIF(convex,i8,i8drct)
           temp(j) = iand(dmsk(j),conf(j,i))
           if (temp(j).ne.holem(j)) go to 10
_ELSEIF(bits8)
          call dand8(dmsk(j),conf(j,i),temp(j))
          if (.not.eq(temp(j),holem(j))) go to 10
_ELSE
          if (.not.eq(dand(dmsk(j),conf(j,i)),holem(j))) go to 10
_ENDIF     
...
_IF(i8drct)
      do ii = 1,5
       do jj = 1,nnst
        con2(ii,jj) = conf(ii,jj)
        con3(ii,jj) = conf(ii,jj)
       enddo
      enddo
_ELSEIF(linux)
      nnst5 = nnst * 5
...
c...    ---vacuum---
      ...
      do 10 in=1,2+nirr*2
         do 9 iset=1,ncmphn(3,in)
            nn = 0
_IF(cray,ksr,i8,i8drct)
            iisym = conf(4,ic+1)
_ELSE
            iisym = ipad(conf(4,ic+1))
_ENDIF
...
_ELSE
               if (iisym.ne.ipad(conf(4,ic))) call caserr('mpini-s1')
_ENDIF
               call upack2(conf(3,ic),ncolt,ncols)
               if (ic.le.nref) then
                  indr(1,ic) = ic
...
_ELSE
                  if (iisym.ne.ipad(conf(4,ic))) call caserr('mpinis2')
_ENDIF
                  call upack2(conf(3,ic),ncolt,ncols)
                  iadv(ic) = nn
                  nn = nn + ncols
...
_ENDIF
               do 27 icon=1,ncmph2(1,in,irconf)
                  ic = ic + 1
_IF(cray,ksr,convex,i8,i8drct)
                  if (iisym.ne.conf(4,ic)) call caserr('mpinis3')
_ELSE
                  if (iisym.ne.ipad(conf(4,ic))) call caserr('mpinis3')
_ENDIF
                  call upack2(conf(3,ic),ncolt,ncols)
                  iadv(ic) = nn
                  nn = nn + ncols
...
c
_IF(i8drct)
      call setstz(nref*5,0,cr(krconf+1))
_ELSE
...
_IF(cray)
         call dcopy(nref1*3,indr,1,icr(kind+1),1)
_ELSE
...
c
_IF(cray,convex,ksr,i8,i8drct)
      isym = conf(4,icst)
_ELSE
      isym = ipad(conf(4,icst))
_ENDIF
...
_ELSE
 120   if (icst.lt.nstrt2.or.ipad(conf(4,icst)).ne.1) return
_ENDIF
c
...
_ELSEIF(convex,i8drct)
      integer *8 conf,rconf,cannil,ccreat
_ELSE
      REAL conf,rconf
_ENDIF
c
...
_ELSEIF(convex,i8drct)
      integer *8 sconf,rconf,cannil
_ELSE
      REAL  sconf,rconf,cannil
_ENDIF
c
...
_ELSEIF(convex)
      integer *8 pad1,pad2,pad3,conf,ref,pad
_ELSEIF(i8drct)
      integer *8 pad1,pad2,pad3,conf,ref,pad
      integer *8 shift
      external shift
_ELSE
      REAL  pad1,pad2,pad3,conf,ref,pad
_ENDIF
      dimension ref(*),conf(*)
      common/dopey/nshif
INCLUDE(common/orb)
...
_IF(cray,ksr)
      iocc = and(shift(ref(kc),ks),pad3)
_ELSEIF(convex,i8)
      iocc = iand(ishftc(ref(kc),ks,64),pad3)
_ELSEIF(i8drct)
      iocc = iand(shift(ref(kc),ks),pad3)
_ELSEIF(bits8)
      call shift8(ref(kc),ks,tmp8)
      call dand8(tmp8,pad3,tmp88)
      iocc = ipad(tmp88)
_ELSE
      iocc = ipad(dand(SHIFT(ref(kc),ks),pad3))
_ENDIF
      if (iocc-1) 10,30,20
...
_ELSEIF(i8drct)
      conf(kc) = ieor(conf(kc),shift(pad,n64-ks))
_ELSEIF(bits8)
      call shift8(pad,n64-ks,tmp8)
      call dxor8(conf(kc),tmp8,conf(kc))
_ELSE
      conf(kc) = dxor(conf(kc),SHIFT(pad,n64-ks))
_ENDIF
c
      return
      end
...
c
        ...
        nuse = i10  + lvr * nv * 2
        call corfai('mp')
_IF(c90,linux,sgi,t3e,j90,sv1)
        call caserr('no quadruple precision schmidt available')
_ELSE
        call qmpschm(1,nv,1,nv,vr,lvr,nv,kept,cortho,iprimp,cr(i10))
_ENDIF
      else if (iortho.eq.4) then
c
...
_IF(cray,convex,ksr,i8,i8drct)
         ...
         jsym= conf(4,nstart)
_ELSE
         isym=ipad(conf(4,mstart))
         jsym= ipad(conf(4,nstart))
_ENDIF
         ksym=mult(jsym,isym)
         nek=norbe(ksym)
         if (nek.eq.0) return
...
c        modified Gramm-Schmidt in quadruple precision (REAL*16)
...
c-----------------------------------------------------------------------
c
_IFN(c90,linux,sgi,t3e,j90,sv1)
      subroutine qmpschm(ia,ib,ja,jb,dv,lv,nv,kept,dschm,ipr,qv)
c
c     ******************************************************************
c
c     NOTE: REAL*16
...
c     ******************************************************************
c
      implicit character*1(a-c,e-h,o-p,r-z)
      implicit REAL (d)
      implicit integer (i-n)
c
c
c     Quadruple Precision Modified Gramm-Schmidt-orthonormalisation
c     =============================================================
c
c     Purpose
c     -------
...
c     dschm : 
c        Criterion for orthogonality, i.e. two vectors are orthogonal 
c        if their inproduct is less than <dschm>.
c
c     ipr   : 
c        Print flag.
c        If ipr > 1 the accuracy of the orthonormalisation will be
...
c
c     qv    :
c        Workspace for the orthogonalisation to store a copy of  dv  in
c        real*16, i.e. quadruple precision.
c
c
c     Theory
c     ------
...
c
      dimension qv(lv,nv)
c     parameter (qzero = 1.0q-15)
c
      dimension dv(lv,nv),kept(nv)
c
      qdepend = 1.0q0 * dschm
      qortho  = 1.0q0 * dschm
      qnorm   = 0.0q0
c
      dsmax   = 0.0d0
c
      if (ia.gt.ja.or.ib.gt.jb) then
        call caserr('orthogonalisation failure in qmpschm !!!')
      endif
c
      do 10 i = 1, nv
        do 20 j = 1, lv
...
c
      do 100 j = ja, jb
c
         if (kept(j).eq.0) go to 100
c
c...     calculate original self overlap
c
         qho1 = 0.0q0
         do 110 l = 1, lv
           qho1 = qho1 + qv(l,j) * qv(l,j)
110      continue
         if (qho1.eq.0.0q0) then
            kept(j) = 0
            go to 100
         endif
c
c...     qv(j) = qv(j) - <qv(i)|qv(j)> qv(i)
c
         do 120 i = ia, min(ib,j-1)
            if (kept(i).eq.1) then
              qs = 0.0q0
...
130           continue
              do 140 l = 1, lv
                qv(l,j) = qv(l,j) - qs * qv(l,i)
140           continue
            endif
120      continue
c
c...     qv(j) = qv(j) / sqrt( <qv(j)|qv(j)> )
c
         qho2 = 0.0q0
         do 150 l = 1, lv
           qho2 = qho2 + qv(l,j) * qv(l,j)
150      continue
         qd = qsqrt(qabs(qho2))
         if (qho2 .le. qho1 * qdepend) then
            kept(j) = 0
            if (qho2 / qho1 .gt. qnorm) then
               qnorm = qho2 / qho1
            endif
         else
            kept(j) = 1
...
c
      do 300 j = ja, jb
         if (kept(j).eq.1) then
            dd = ddot(lv,dv(1,j),1,dv(1,j),1)
            dd = 1.0d0 / dsqrt(dd)
            call dscal(lv,dd,dv(1,j),1)
         endif
300   continue
c
c...  find maximum overlap
c
      ...
      do 200 j = ja, jb
         if (kept(j).eq.1) then
            do 210 i = ia, min(ib,j-1)
               if (kept(i).eq.1) then
                  ds = dabs(ddot(lv,dv(1,j),1,dv(1,i),1))
                  if (ds.gt.dsmax) then
                     dsmax = ds
                  endif
               endif
...
200   continue
      if (ipr.gt.1.or.qnorm.lt.1.0d-10) then
         write(*,*)'*** QUADRUPLE PRECISION MODIFIED GRAMM-SCHMIDT'
         write(*,*)'Maximum norm coeff. dropped = ',qnorm
         write(*,*)'Maximum overlap             = ',dsmax
      endif
      if (dsmax.gt.dschm) then
        write(*,900)dsmax,dschm
900     format(' ** Maximum overlap ',e12.5,' exceeds threshold ',e12.5,
     *         ' in quadruple precision modified Gramm-Schmidt !!!')
        call caserr("Maximum overlap exceeds threshold !!!")
      endif
c
      return
...
c      Section   5.1.6 (Backward accumulation of Q) 
c
c     Purpose
c     -------
c
...
c
      if (ifase .lt. 3) goto 200
c
c...  Othogonalisation is done now accumulate Q from Householder vectors
c
      do 500 it = 1, lv
         do 510 jt = 1, lv
            q(jt,it) = 0.0d0
...
_IF()
      subroutine diisci(c,z,indix,ntotal,indc,indz,energy,
     *                  conf,ifcepa,nref)
c
      implicit REAL (a-h,o-z), integer(i-n)
c
      dimension c(ntotal),z(ntotal),indix(3,2),conf(5,*)
c
...
c...   in DIRECT indix is probably index(31-36)
c...   indc/indz : address for c/z vector
...
c
        call rdedx(c,ntotal,indix(1,1),numci)
        call rdedx(z,ntotal,indix(1,2),numci)
        if (ifcepa.gt.0) call cepvds(conf,z,z,c,.false.,.true.)
...
c
        ...
        write(6,989) h11pre,energy
 989    format(11x,'pre DIIS energy ',f18.12,' DIIS energy ',f18.12)
c
      return
c
      entry dinici
...
c
      if (pplev.ne.0.0d0)
_IF(ibm,vax)
     *   call vsav(ntotmp,-pplev,cr(kmpz+1),cr(kmpz+1))
_ELSE
...
c
      if (pplev.ne.0.0d0)
_IF(ibm,vax)
     *   call vsav(ntotmp,-pplev,cr(kmpc+1),cr(kmpc+1))
_ELSE
...
_ELSEIF(bits8)
               call pad8(kk,conf(1,loop))
               call pad8(nex,conf(3,loop))
               call pad8(1,conf(4,loop))
               call pack2(iex-6,1+nhole,conf(5,loop))
_ELSE
                  conf(1,loop) = pad(kk)
                  conf(3,loop) = pad(nex)
                  conf(4,loop) = pad(1)
...
_ELSEIF(bits8)
               call pad8(ll,conf(1,loop))
               call pad8(nex,conf(3,loop))
               call pad8(ijrr,conf(4,loop))
               call pack2(iex-6,4+nhole,conf(5,loop))
_ELSE
                     conf(1,loop) = pad(ll)
                     conf(3,loop) = pad(nex)
                     conf(4,loop) = pad(ijrr)
                     conf(5,loop) = pack2(iex-6,4+nhole)
_ENDIF
                     ll = ll + nex*norex
                  end if
...
_ELSEIF(bits8)
               call pack2(kkk,kkk,conf(1,loop))
               call pack2(ntex,nsex,conf(3,loop))
               call pad8(ijrr,conf(4,loop))
               call pack2(iex-6,7+nhole,conf(5,loop))
_ELSE
                     conf(1,loop) = pack2(kkk,kkk)
                     conf(3,loop) = pack2(ntex,nsex)
                     conf(4,loop) = pad(ijrr)
...
_IF(cray,ksr,i8,i8drct)
                  ...
                  conf(5,loop) = pack2(iex-6,7+nhole)
_ELSEIF(bits8)
               call pack2(0,kkk,conf(1,loop))
               call pack2(0,lll,conf(2,loop))
               call pack2(0,nex,conf(3,loop))
               call pad8(1,conf(4,loop))
               call pack2(iex-6,7+nhole,conf(5,loop))
_ELSE
                  conf(1,loop) = pack2(0,kkk)
...
_ELSE
        call skipsb(lams*ipad(conf(3,ir)))
_ENDIF
         goto 22
      end if
...
_IF(ibm,vax)
            call vsav(ioop,scr(idbas+joop),diag(icbas+1),diag(icbas+1))
_ELSE
...
_IF(cray)
708   call dcopy(nirr,multtt(1,isi),1,multt,1)
_ELSE
...
_IF(cray,ibm,vax)
               call vsav(ne,scr(iff+joop),diag(icbas+1),diag(icbas+1))
_ELSE
...
_IF(cray,ibm,vax)
                call vsav(joop-1,cr(idbas+joop),cr(icbas+1),cr(icbas+1))
_ELSE
...
_IF(cray,ibm,vax)
                  call vsav(ne,cr(iff+joop),cr(icbas+1),cr(icbas+1))
_ELSE
...
_IF(notused)
c=====================debug-routines=================================
      subroutine printr(a,n)
      implicit REAL (a-h,o-z)
      dimension a(n)
      write(6,1) a
1     format(1x,10f11.5)
      end
      subroutine prvec(text,v,ndim)
c
      implicit REAL (a-h,o-z)
c
c...  print a (ci/mp) vector
c
      character*(*) text
      dimension v(ndim)
c
      ll = len(text)
      write(6,'(1x,5(1h=),2x,a,2x,5(1h=))') text(1:ll)
      write(6,'(1x,10f11.5)') v
c
      return
      end
_ENDIF
_IF(cray,ksr,convex,i8drct) 
c...   this is only for the 8-byte integer machines ... 
      integer function ipad(conf)
_IF(cray,ksr)
      integer   conf
_ELSE
      integer *8   conf
...
c...     vmin mode
         ...
         call jacobi(f,iky,joop,q,ilifq,joop,eig,2,1,1.0d-9)
_IF(cray,ibm,vax)
         call vsav(joop,h11*h11,eig,eig)
_ELSE
...
INCLUDE(common/dir_erg)
      dimension conf(5,*),cr(*)
      REAL Ai,Bi,Ci,Aa,Ba,Ca,ph_pair(3,3)
      integer Ni,Na
c
      character*6 txt(3)
