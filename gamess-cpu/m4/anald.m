...
c..  sort on real parts of eigenvalues
      ifail = 0
_IF(c90,convex)
      call m01daf(rr,1,nend,ascend,intger,ifail)
_ELSE
...
c...  mid-point using omega = w0 tan t
 80   do 90 i = 1 , nc6
         w(i) = w0/((dcos(dfloat(2*i-1)*pi/(dfloat(4*nc6))))
     +          **2*dfloat(4*nc6))
 90   continue
c
 100  write (iwr,6030)
...
_IF1(iv)         call upak4v(g(num2ep+1),labij)
         do 60 i = 1 , nword
_IFN1(iv)            lab = labs(i+i-1)
_IFN1(iv)            if (lab.eq.labs(i+i)) then
_IF1(iv)            lab = labij(i)
_IF1(iv)            if (lab.eq.labkl(i)) then
...
c
 ...
 20      continue
         ...
         if (nreq.le.maxq .and. npd.ne.0) then
            ...
            call trksum(eps,Q(i1),Q(i2),npd,mn)
            call gmem_free(i2)
            call gmem_free(i1)
            call timit(3)
            irestp = 2
...
c
 ...
 40         mv = mv - 1
            ...
            i10 = igmem_alloc(mv)
            i11 = igmem_alloc(mv)
            call tdchf1(eps,Q(idiag),Q(i2),Q(i3),
     +           Q(i4),Q(i5),Q(i6),Q(i7),
     +           Q(i8),Q(i9),Q(i10),Q(i11),
     +           mn,mv)
            call timit(3)
            call gmem_free(i11)
            call gmem_free(i10)
...
c
         ...
         call tdchf0(eps,Q(i1),Q(i2),Q(i3),
     +        Q(i4),Q(i5),mn)
         call gmem_free(i5)
         call gmem_free(i4)
         call gmem_free(i3)
...
c
 ...
 60      if (mv.le.mvmin) then
            ...
            call tdchf2(eps,Q(idiag),Q(i2),Q(i3),
     +           Q(i4),Q(i5),Q(i6),
     +           Q(i7),Q(i8),Q(i9),mn,mv)
            call gmem_free(i9)
            call gmem_free(i8)
            call gmem_free(i7)
            call gmem_free(i6)
            ...
            call polasm(Q(i2),Q(i3),Q(i4),mn,npdim)
            call gmem_free(i4)
            call gmem_free(i3)
            call gmem_free(i2)
