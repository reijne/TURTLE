     ...
     1           nxbkt,newlen,npercl,itap91,lpercl,irew)
      ...
      real*8 buf(nszbf),rints(newlen*npercl)
...
c
     ...
     1                 itap60,itap62,itap63,ni,nj,nk,nl,no,enuc)
      ...
      real*8 buf(lenbf),h(ntri),eig(nbf),escf,enuc,a0
...
c
      ...
      nj(ind) = and(itemp,ishft(ibuf(2+ind),-16))
      nk(ind) = and(itemp,ishft(ibuf(2+ind),-8))
...
c     write (jout,*) ' mbuf=',mbuf
      do 101 ii=1,mbuf
         ...
         i=ishft(ijkl,-24)
         j=and(itemp,ishft(ijkl,-16))
         k=and(itemp,ishft(ijkl,-8))
         l=and(itemp,ijkl)
         rint=buf(iboff+ii)
         i=i-no
         k=k-no
...
c
         ...
         iposik=indxvl(ik)
         iposjl=indxl(jl)
         jlsym=ieor(orbsym(j),orbsym(l))+1
...
c
      integer pos
      common /t_pointr/ pos(100)
c
c
      integer sector
      common /t_sect/ sector
c
c
      character *4 fname
      common /t_flname/ fname(100)
...
c
      do 10 i = 1,icnt
      nj(i)=or(idu(nl(i)),ishft(or(idu(nk(i)),ishft(nj(i),1)),1))+1
 10   continue
c
c  pack k and l into nk
c
      do 20 i = 1,icnt
      nk(i) = or(nl(i),ishft(nk(i),8))
 20   continue
c
c  pack ijkl into nl
c
      do 30 i =1,icnt
      nl(i) = or(min(ni(i),nk(i)),ishft(max(ni(i),nk(i)),16))
 30   continue
c
...
c
     ...
     &           itap65,itap66,itap67,itap69,itap78,itap91,itap57)
      implicit integer (a-z)
      real*8 f(ntris),h(ntri),buf(lnbuf),buf2(lnbuf),bkt(ibktsp)
      real*8 enuc
      integer orbsym(nbf)
...
c
      real*8 potnuc
      integer nsym, nocc, nvir, lu1el, ad1el, lu2el, ad2el
      common /titan/ potnuc,nsym,nocc(8),nvir(8),lu1el,ad1el,lu2el,ad2el
c
c
      icnt=0
      do isym=1,nirred
...
10    continue
      ...
      do 12 i=1,nv
      do 22 j=1,nv
      totsym=ieor(orbsym(i+no),orbsym(j+no))
...
c
 ...
 5    norbo(i)=flov(i,2)-flov(i,1)+1
      ...
      do 12 i=1,no
      do 22 j=1,nv
      totsym=ieor(orbsym(i),orbsym(j+no))
...
c
      ...
      do 12 i=1,nv
      do 22 j=1,no
      totsym=ieor(orbsym(i+no),orbsym(j))
      norsq(totsym+1)=norsq(totsym+1)+1
22    continue
12     continue
      call tit_izo(nvrsq,nirr)
...
c
      ...
      iposij=indxol(ij)
      iposkl=indxol(kl)
      ijsym=ieor(orbsym(no+i),orbsym(j))+1
...
c
      ...
      iposil=indxol(il)
      iposjk=indxol(jk)
      jksym=ieor(orbsym(j),orbsym(k+no))+1
      ilsym=ieor(orbsym(no+i),orbsym(l))+1
...
c
      ...
      iposik=indxl(ik)
      jlsym=ieor(orbsym(j),orbsym(l))+1
      iksym=ieor(orbsym(no+i),orbsym(no+k))+1
...
c
      ...
      iposij=indxol(ij)
      iposkl=isqoo(kl)
      klsym=ieor(orbsym(k),orbsym(l))+1
...
c
      ...
      iposil=indxol(il)
      iposjk=isqoo(jk)
      jksym=ieor(orbsym(k),orbsym(j))+1
...
c
      ...
      iposik=isqvv(ik)
      iposjl=indxol(jl)
      jlsym=ieor(orbsym(i+no),orbsym(k+no))+1
...
c
      ...
      iposil=indxol(il)
      iposjk=isqvv(jk)
      jksym=ieor(orbsym(k+no),orbsym(j+no))+1
...
c        write (jout,12)
         ...
         k=imap(k)
         l=imap(l)
         ijkl2 = or(j,ishft(i,8))
         ijkl2 = or(k,ishft(ijkl2,8))
         ijkl2 = or(l,ishft(ijkl2,8))
...
c
         ...
         iposik=indx(ik)
         iposjl=indxl(jl)
         jlsym=ieor(orbsym(no+j),orbsym(no+l))+1
         iksym=ieor(orbsym(no+i),orbsym(no+k))+1
...
c
         if(k.ne.l) then
            ...
            iposil=indx(il)
            iposjk=indxl(jk)
            jksym=ieor(orbsym(no+j),orbsym(no+k))+1
            ilsym=ieor(orbsym(no+i),orbsym(no+l))+1
...
c
      ...
      iposjl=indxol(jl)
      jlsym=ieor(orbsym(j),orbsym(l))+1
      iksym=ieor(orbsym(i),orbsym(k))+1
      iadr=(iposik-1)*norsq(jlsym)+iposjl+iofosy(jlsym)
...
c
      ...
      iposil=indxol(il)
      iposjk=indxol(jk)
      jksym=ieor(orbsym(j),orbsym(k))+1
      ilsym=ieor(orbsym(i),orbsym(l))+1
      iadr=(iposil-1)*norsq(jksym)+iposjk+iofosy(jksym)
      ibkt1=psobkt(iposil+iofosq(jksym))
      ixbkt=(ibkt1-1)/npercl+1
      ibit=mod(ibkt1-1,npercl)
...
c
      ...
      iposik=indxol(ik)
      iposjl=indxol(jl)
      jlsym=ieor(orbsym(j+no),orbsym(l))+1
      iksym=ieor(orbsym(i+no),orbsym(k))+1
      iadr=(iposik-1)*norsq(jlsym)+iposjl+iofosy(jlsym)
...
c
      ...
      iposil=indxol(il)
      iposjk=indxol(jk)
      jksym=ieor(orbsym(no+j),orbsym(k))+1
      ilsym=ieor(orbsym(no+i),orbsym(l))+1
      iadr=(iposil-1)*norsq(jksym)+iposjk+iofosy(jksym)
      ibkt1=psobkt(iposil+iofosq(jksym))
      ixbkt=(ibkt1-1)/npercl+1
...
c
      ...
      iposjl=indxol(jl)
      iposik=indxol(ik)
      jlsym=ieor(orbsym(no+j),orbsym(l))+1
      iksym=ieor(orbsym(no+i),orbsym(k))+1
      iadr=(iposjl-1)*norsq(iksym)+iposik+iofosy(iksym)
      ibkt1=psobkt(iposjl+iofosq(iksym))
      ixbkt=(ibkt1-1)/npercl+1
      ibit=mod(ibkt1-1,npercl)
...
c
     ...
     1  ioff(nbf),itriv(ntriv),isqoo(no*no)
      real*8 e(nbf),buf(2),d,emp2,empx,escf
      data a2 /2.0d0/
c
