...
_IF(parallel)
      call pg_dlbpush
      nbsq = nb*nb
      if (odebug(61)) write (iwr,151) mvec
151   format ('performing global sums of ',i3,' u matrices')
      do 160 m = 1 , mvec
         call pg_dgop (600,u1(1,1,m),nbsq,'+')
         call pg_dgop (600,u2(1,1,m),nbsq,'+')
 160  continue
_ENDIF(parallel)
c
c     free memory
...
_IF(parallel)
INCLUDE(common/parallel)
      common/pardir/next,nbcnt(256),loadb(256)
_ENDIF(parallel)
...
_IF(parallel)
               call pg_brdcst(1300,q(idiag),isize2*8,0)
_ENDIF(parallel)
               end if
            else
...
_IF(parallel)
         m = min(ipg_nnodes(),256)
         nbtot = isum(m,loadb,1)
         write (iwr,210) (i,i=1,m)
210      format (/
     +     'percentage of integral batches distributed over the nodes'
     ?    /'(averaged over all iterations):'//32i7)
         write (iwr,211) (100.0d0*dble(loadb(i))/dble(nbtot),i=1,m)
211      format (32f7.1)
_ENDIF(parallel)
...
c   r = omega * s - e
         ...
         freq = etare(j)
_IFN1(civu)         call vsmsb (v(n1),1, freq,e(n1,1),1,e(n1,1),1,isize)
_IFN1(civu)         call vsmsb (v(n2),1,-freq,e(n2,1),1,e(n2,1),1,isize)
_IF1(civu)         call gtrian(isize,freq,e(n1,1),e(n1,1),v(n1))
_IF1(civu)         call gtrian(isize,-freq,e(n2,1),e(n2,1),v(n2))
         if (odebug(47)) then
            write (iwr,61) j
...
_IF(parallel)
c
c   estimate load balancing factor for parallelized direct RPA
c
...
_IF(parallel)
      m = min(ipg_nnodes(),256)
      call pg_igop (333,nbcnt,m,'+')
      if (odebug(62)) call outive (nbcnt,m,'nbcnt')
      do iloop = 1,m
         loadb(iloop) = loadb(iloop) + nbcnt(iloop)
      enddo
_ENDIF(parallel)
