...
c
            do 110 i = 1 , ncoord
               call vmul(vec(1,i),1,rm,1,vec(1,i),1,ncoord)
 110        continue
            do 120 j = 1 , ncoord
...
c
         ...
         if (no2.gt.0) then
            do 50 i = 1 , no2
               ...
               eig(i) = eig(m)
               eig(m) = t
             call dswap(n,vec(1,i),1,vec(1,m),1)
 50         continue
         end if
c
c    invert order of +ve roots
...
c
c        gg = ddot(n,g,1,g,1)
         cos12 = ddot(n,tans,1,tan2,1)
     +           /dsqrt(ddot(n,tans,1,tans,1)*ddot(n,tan2,1,tan2,1))
c
         ineg = 1
c
...
c     Parameter:
c
      real*8 linearity_tolerance ! cutoff for linear molecules 
      parameter(linearity_tolerance=1.0d-1)
c
c     Input:
c
      real*8 pmom(3) ! The moments of inertia
c
c     Local:
c
      real*8 sn
...
c     Functions:
c 
      real*8 symnum   ! function checking linearity against point group
