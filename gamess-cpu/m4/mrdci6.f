...
49    call vclr(f,1,nzer)
      ...
      go to 43
42    tim=-bob(256-mike)
43    call daxpy(kmj,tim,ae(kx),ndj,f(m),kml)
40    continue
47    mz=kz
      call mxmaa(ae(ie+1),kml,1,f,1,kml,res,1,kml,kml,kml,kmj)
      do 45 m=1,kml
...
58    tim=sm
      kx=my+ja+ndj
59    call daxpy(kmj,tim,ae(kx),ndj,f(m),kml)
56    continue
      go to 47
  33  mm=mm+1
      ll=olab(mm)
...
78    call daxpy(kmj,tim,ae(kx),ndj,f(m),kml)
      go to 73
 75   in=in+1
      kx=ot(in)
      kx=kx+ja+ndj
...
124   tim=tim-aexc(nal)
      ml=mx
122   call daxpy(kml,tim,ae(kx),ndt,f(l),kml)
...
191   jj=jj+1
      ...
      nn=ot(in)
197   tim=tim+sac(nn)
196   call daxpy(kml,tim,ae(kx),ndt,f(l),kml)
...
195   if(nms.eq.1) go to 200
      ...
      nn=ot(in)
201   tim=tim+sac(nn)
200   call daxpy(kml,tim,ae(kx),ndt,f(l),kml)
      do 203 m=1,nps
      in=in+1
      kx=ot(in)
      kx=kx+ia+ndt
...
215   jj=jj+1
      ...
      nn=ot(jn)
221   tim=tim-sac(nn)
220   call daxpy(kml,tim,ae(kx),ndt,f(l),kml)
