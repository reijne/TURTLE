...
c
c
      real*8 eps, epss, epsana, epstab, sytol, tolmy, fetda, ferpa
      real*8 tottda,totrpa
      integer modus, nevlo, nevhi, nev, maxr, maxit, maxrr, maxitt
      integer  nbegin, ipref,iblo13
...
c
     ...
     +   v1(1,j),v2(1,j),h1,h2,scr)
      tim(2) = tim(2) + cpulft(1) - time
      freq = etare(j)
      call vsmsb(v1(1,j),1,freq,h1,1,h1,1,isize)
      call vsmsb(v2(1,j),1,-freq,h2,1,h2,1,isize)
      if (odebug(47)) then
         write (iwr,61) j
...
c   starting vector
c
               call vsadd(diag,1,freq(i),q(ih1),1,isize)
     ...
     +            call caserr ('vector f equals zero !')
               ...
               call vsadd(diag,1,-freq(i),q(ih2),1,isize)
