...
c off-diagonal tape construction
      ...
      nbufo=nbufo+1
      call icopy(nwor*nav,nuwko,1,ijkl,1)
      call ed0out
      nelino=0
      goto 30
c
...
c diagonal tape construction
      ...
      nbufd=nbufd+1
      call icopy(nwor*nav,nuwkd,1,ijkl,1)
      call ed0out
      nelind=0
30    continue
c
...
c
      ...
      do 200 i=1,nblks
      call getblk (ed7,nw)
      call icopy(i2*nav,nuwks,1,lout,1)
      if (i.ne.1) goto 200
      ndiag1 = ndiag/65536
...
c
      ...
      nwbnwb=nwb
      lnklnk=mark(ibuck)
      call pack(iyout,16,ibuff,ibl5*4)
      call sttout
      mark(ibuck)=iblock
      nwb=0
      iblock=iblock+nsz
...
c
      ...
      call rdbak(mkkkk)
      call stopbk
      call unpack(iyout,16,lin,ibl5*4)
      return
      end
      subroutine ed0out
      implicit real*8  (a-h,p-w),integer (i-n),logical  (o)
...
c
      real*8 pop, popsqm, popsqn
      common /popnos/ pop(100),popsqm(100),popsqn(100)
c
c
      integer kkkm, kkk, idom, iocca, ioccb, irestu, ifstr
      real*8 vl
      common/exc/kkkm,kkk,vl(5),idom(5),iocca(5,15),ioccb(5,15),
     +      irestu,ifstr
c
c
      integer n20cas
...
c
      vc=vc*vt
      vc2m=-vc*2.0d0
      call vsmul(grad,1,vc2m,h,1,kk10)
...
c160  q(isig+irow)=brill(irow)*q(icc)
      call vsmul(brill,1,q(icc),q(isig+1),1,n)
      icc=icc+np4
170   isig=isig+np4
c
      do 180 irow=1,n
...
c     brill(i)=buffio(i+1)/(-buffio(1))
      test=-1.0d0/buffio(1)
      call vsmul(buffio(2),1,test,brill,1,n)
...
240    format(/' diagonalisation converged in',
     ...
     2    f8.2,' seconds')
      i4 = nroot - nstate
      i4 = max(i4,1)
      vk1  = vk10 / dsqrt( dble(i4))
      vk2  = vk20 / dsqrt( dble(i4))
      jm = nroot*2
...
c     orthogonalisation
c
      implicit real*8  (a-h,p-x),integer (i-n),logical  (o)
...
c.....expand exponential
      ...
      do 30 iterm=2,nterm
      scaler=done/ dble(nterm+2-iterm)
      do 30 i=1,n
      call vsmul(u(i,1),ndim,scaler,vec,1,n)
      vec(i)=vec(i)+done
      do 30 j=1,n
...
c
      do 110 i=1,n
      do 90 j=1,n
      vec(j)=ddot(n,u(i,1),ndim,w1(1,j),1)
 90   continue
      do 100 j=1,n
...
c
      buff= dble(n4)/four
      maxvec =  dsqrt(buff**2+( dble(max)/two)) - buff
      i4 = n + 1
...
c
c.....trial solution
      call vmul(brill,1,diag,1,q(ic),1,n)
...
c
      ...
      mword=mword+1
      gout(mword)=gam
      ij205(int4  )=i4096(it)+iu
      ij205(int4+1)=i4096(i)+i
      int4=int4+2
...
30    mword=mword+1
      ...
      ij205(int4  )=i4096(it)+i
      ij205(int4+1)=i4096(iu)+i
      int4=int4+2
...
c
     ...
     + nopkr,iofsym,iofrst,idurie(2),ognore,imaxb_ic
      if(ipos(mainp).ne.iblkmp)callsearch(iblkmp,mainp)
      call pack(gout(num2ep+1),lab1632,ij205,numlabp)
