...
c
 ...
 440  continue
      ...
      nend=nend+1
      s=1.0d0/s
      call vsmul(buffer(1,ibase+k),1,s,buffer(1,nend),1,kk3)
c
c
460   continue
c
...
c..... now do the transformation
      ...
      lenj=jfin-jstart+1
      call mxmaa(q(ilifm(jstart)+1),lenl,1,p,1,lenl,v(jstart),1,lenj,
     1                 lenj,lenl,1)
c.... put result vector y into output buffer
      ...
      gout(mword)=v(j)
      int4=int4+2
      kbcray(int4  )=mapie(j)+mapk
      kbcray(int4+1)=master
      enddo
      if(mword.ne.nintmx)goto230
      call pack(gout(num2ep+1),lab1632,kbcray,numlabp)
      int4=-1
      call blockk
      jfinb1=jfinb+1
      do j=jfinb1,jfin
...
270   lenj=jfin+1-jfirst
      ...
      if(lenj.ge.1)call mxmaa(q(ilifm(jfirst)+1),lenl,1,p,1,lenl,
...
c    * indxj,top
      gout(mword)=top
      int4=int4+2
      kbcray(int4  )=mapbk+i4096(mapbak(j))
      kbcray(int4+1)=mword1
...
c     write(6,6000)nst,nfi,k,isymk,isyml,jend,jfirst,lenj
      call mxmaa(q(ilifm(jfirst)+1),lenl,1,p,1,lenl,v(jfirst),1,lenj,
     1           lenj,lenl,1)
      mapk=i4096(mapbak(k))
...
c     write(6,6003)j,jstart,last,indxi,indxj,mapbak(j),mapk,top
      int4=int4+2
      kbcray(int4  )=mword1
      kbcray(int4+1)=mapk+i4096(mapbak(j))
