...
c
     ...
     + goto 777
      if (icyc.ge.maxcyc) goto 440
          tlefti = timlim - top
         if (.not.(tlefti.ge.2.0d0*(tlefts-tlefti)/dble(icyc))) 
     + goto 440
c
...
c*ap     use of integer *and*
      if (and(et(i),1).eq.1) sym = mult(sym,i)
30    continue
c
...
c
 ...
 20   istak = ld340(buf,ijklbuf)
      if (istak.ne.0) then
        ...
        ibn=ibasen(istak)+1
        ib = lnumb(istak) + 1
        call pack(buf2(nsz341),lab1632,ijklbuf(ibn),nsz680)
