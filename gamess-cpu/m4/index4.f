...
c
     ...
     +                call daxpyi(nzero,vec(ki,i+if1),triag,map,
     +                aiqrs(1,kj,i))
                  if ((vec(kj,i+if1).ne.0.0d0) .and. ki.ne.kj)
     +                call daxpyi(nzero,vec(kj,i+if1),triag,map,
     +                            aiqrs(1,ki,i))
 30            continue
 40         continue
 50      continue
...
c
     ...
     +                         (kf1.eq.if1 .and. lf1.gt.jf1)) then
                           ...
                           iw2 = mword + mword
                           mwcray(iw2-1) = jf1 + i4096(if1)
                           mwcray(iw2) = lf1 + i4096(kf1)
...
c
     ...
     +                               lf1 , top
                                 ...
                                 gxoxx(mwordz) = top
                                 iw2z = mwordz + mwordz
                                 nwcray(iw2z-1) = jf1 + i4096(if1)
                                 nwcray(iw2z) = lf1 + i4096(kf1)
                                 if (odebug(11)) write(iwr,*)
...
c     endif
      call pack(gout(num2ep+1),lab1632,labs,numlabp)
      nint = n
      len = m511
      if (n.eq.0) len = m0
...
c
      ...
      call pack(gout(num2ep+1),lab1632,labout,numlabp)
