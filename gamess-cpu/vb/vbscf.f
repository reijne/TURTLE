...
c
     ...
     &                           IGMEM_DEBUG)
      call get1e(qq(ivoff+ks),dummy,'s',qq(ivoff+kscr))
      if (ofirst.and.iprins.gt.50) then
         if (iprins.gt.10000) then
            write(iwr,1000)
1000        format(/,' the ao metric :')
            call tripri(qq(ivoff+ks),nbasis)
         end if
         call sigorb(qq(ivoff+ks),nbasis)
      end if
c
...
c
      ...
      call normt(v,nbasis,ncol,qq(ivoff+ks),qq(ivoff+kscr))
...
c      call prvc14(v,mm,nbasis,v,'o','l')
      if (ortscf) 
     &   call vbcanon(v,nbasis,ncore+nactiv,100,'occ',qq(ivoff+1))
*
c...     make sure equivalence restrictions are obeyed 
c
...
c
        call restror(v(ncore*nbasis+1),nsa,nbasis,qq(ivoff+1))
*
c
...
*       the s matrix (zahid) 
*
        call get1e(qq(ivoff+ksao),dumm,'s',qq(ivoff+ksao))
*
        call redorb(qq(ivoff+ksao),v(ncore*nbasis+1),qq(ivoff+kvcopy),v,
     +              qq(ivoff+kiocvi),qq(ivoff+kiset),qq(ivoff+kiact),
     +              qq(ivoff+kidoc),qq(ivoff+kisoc),qq(ivoff+kivir),
     +              qq(ivoff+khmoao),qq(ivoff+kiex2),nbasis,qq(ivoff+1))
        ncol = ncol2 - nredund
        nsa = ncol - ncore
        nvirt = nsa - nactiv
...
c
        call transformvb(qq(ivoff+ks),qq(ivoff+kh),v)
...
c
        ...
        call bsmat(qq(ivoff+ks),qq(ivoff+kh),qq(ivoff+kg),
...
c
     ...
     &                        IGMEM_DEBUG)
        ...
        call davids(qq(ivoff+kbi),kmemscr,core,ebi,'bi')
        a1 = cpulft(1) - a1
        tdavid = tdavid + a1
...
c...  move variational vector
           call fmove(qq(ivoff+kbi),qq(ivoff+kciint),nvarp)
           call vbper(qq(ivoff+kciint),qq(ivoff+kbi),qq(ivoff+kgrh),
     +                qq(ivoff+kgrs),qq(ivoff+kdiagh),qq(ivoff+kdiags),
     +                qq(ivoff+kigr_brill))
...
c...  change orbitals
c
        call orbopt(v,nbasis,nactiv,qq(ivoff+knewv),qq(ivoff+kbi),
     +              qq(ivoff+kscr),kmemscr)
        call gmem_free_inf(kscr,'vbscf.m','vbscf','kscr')
...
*
     ...
     +                          'kscr',IGMEM_DEBUG)
*           
           call get1e(qq(ivoff+ksao),dummy,'s',qq(ivoff+kscr))
...
*              
     ...
     +                                IGMEM_DEBUG)
*              
           call normvc(v(ncore*nbasis+1),qq(ivoff+ksao),qq(ivoff+kscr),nbasis,
     +                 nactiv,cridep)
*              
           call gmem_free_inf(kscr,'vbscf.m','vbscf','kscr')
           call gmem_free_inf(ksao,'vbscf.m','vbscf','ksao')
...
c
      call get1e(qq(ivoff+ksao),dummy,'s',qq(ivoff+ksao2))
      call fmos(qq(ivoff+ks),qq(ivoff+ksao),v(ncore*nbasis+1),
     +          qq(ivoff+kscr),nsa,nbasis,crilow)
...
c
     ...
     &                       IGMEM_DEBUG)
*
      call scfina(v,ci,qq(ivoff+ks),qq(ivoff+kscr),qq(ivoff+1))
*
...
c
     ...
     &                       IGMEM_DEBUG)
      call sym1(qq(ivoff+ks),norb,qq(ivoff+kiort),northo,'noprint')
...
c
      call inform(nelec,nalfa,ndets,nstruc,qq(ivoff+kpacd),
     &            qq(ivoff+kndet),qq(ivoff+kidps),qq(ivoff+kcoef),
     &            ncoeff,qq(ivoff+kigro),ngroup,qq(ivoff+kiort),
     &            northo,qq(ivoff+kscr),kmemscr)
c
c...  generate psi0 on determinant basis
c
      call psi0det(ci,nstruc,qq(ivoff+kigro),ngroup,
     &             qq(ivoff+kcoef),ncoeff,ndets,qq(ivoff+kndet),
     &             qq(ivoff+kidps),qq(ivoff+kidet),qq(ivoff+kjdet),
     &             qq(ivoff+kpacd),nelec,nalfa,ndettot,
     &             qq(ivoff+kscr),kmemscr,
     &             'dontsaveondisk')
c
...
c
     ...
     &                         IGMEM_DEBUG)
      call bmain(qq(ivoff+kpacde),qq(ivoff+kidet),qq(ivoff+kjdet),
     &           qq(ivoff+kdetco),qq(ivoff+kdetto),
     &           qq(ivoff+kicp),qq(ivoff+kjcp),qq(ivoff+kidetp),
     +           qq(ivoff+kncoef),qq(ivoff+kig),
     &           qq(ivoff+kigrou),qq(ivoff+khamil),qq(ivoff+koverl),
     &           supers,superh,superg,
     +           qq(ivoff+kipos),qq(ivoff+kweigh),
     &           qq(ivoff+kg),qq(ivoff+kiorth),qq(ivoff+ks),
     +           qq(ivoff+kscr1),qq(ivoff+kscr2),
     &           qq(ivoff+kscr3),qq(ivoff+kndetp),
     &           ciref,qq(ivoff+kscr),kmemscr,nvirt)
...
c
      ...
      do 30 i=1,na
         call cntrc(s,a(1,i),qq(ivoff+kw),nbasis,critor/100.0d0)
         if (ipar.eq.0) then
c....       a and b are the same set
         ...
         do 20 j=jstart,nb
            if (ipar.eq.0) then
               aa = ddot(nbasis,qq(ivoff+kw),1,a(1,j),1)
            else
               aa = ddot(nbasis,qq(ivoff+kw),1,b(1,j),1)
            end if
...
c
     ...
     @                     IGMEM_DEBUG)
      iseta = idepen(a,qq(ivoff+kvc),s,qq(ivoff+kw),nbasis,na)
      write(iwr,22)
22    format(' first set ')
...
55       format(' second set')
         ...
         isetb = idepen(b,qq(ivoff+kvc),s,qq(ivoff+kw),nbasis,nb)
         if (isetb.eq.0) then
            write(iwr,66)
...
c
     ...
     &                            'kvecmt',IGMEM_DEBUG)
         call lowdins(v,s,qq(ivoff+ks),qq(ivoff+kcmsq),
     &                qq(ivoff+klambda),qq(ivoff+kvecmt),
     &                ndoc,nbasis,crilow)
...
c
     ...
     &                     'kw',IGMEM_DEBUG)
      call schmids(v(1,ndoc+1),v,s,qq(ivoff+kw),nsoc,ndoc,
     &             nbasis,cridep,'norm')
...
c
     ...
     &                       'khao2',IGMEM_DEBUG)
      call get1e(qq(ivoff+khao),dummy,'v',qq(ivoff+khao2))
      call gmem_free_inf(khao2,'vbscf.m','reexc','khao2')
      if (hybry) call clmetr(qq(ivoff+khao),nao,'any')
      nam=nao+nmo
      kscr2 = igmem_alloc_inf(nam*nam+nam*nao,'vbscf.m','reexc',
     &                        'kscr2',IGMEM_DEBUG)
      kscr3 = igmem_alloc_inf(nam*nao,'vbscf.m','reexc',
     &                        'kscr3',IGMEM_DEBUG)
      call mult11(qq(ivoff+khao),haomo,qq(ivoff+kscr2),
     &            nam,nao,vec,qq(ivoff+kscr3))
c
...
c
     ...
     &                        'khaomo',IGMEM_DEBUG)
      call fmove(haomo,qq(ivoff+khaomo),nam*(nam+1)/2)
      if (nosymm) goto 666
c
         ...
         if (nexeq(i).gt.0) then
            ...
            do j=1,nn
               ...
               do k=1,iequi(i)
                  ...
                  iex(iii,iorbn) = im
                  qq(ivoff+khaomo-1+ind(iorbn,im)) = 0.0d0
                  ii = ii + 2
               end do
            end do
...
c
         do 30 j=nmo+1,nmo+nao
            if (idequi.eq.2) then
               call abmax(qq(ivoff+khaomo),
     &                    nmo+nao,nmo+1,'tri',iorb1,am,im)
            else
               im = j
            end if
            internal = im.le.2*nmo+ncore
            h1=dabs(qq(ivoff+khaomo-1+ind(iorb1,im)))
            if (h1.gt.remcri) then
               if (h1.lt.lowesth1) lowesth1 = h1
               iii=iii+1
               iex(iii,iorb1)=im
               qq(ivoff+khaomo-1+ind(iorb1,im)) = 0.0d0
               do 40 k = 2,iequi(i)
                  iorb2 = iorb1+k-1
                  if (idequi.eq.2) then
                    naomo = nmo + nao
                    if (im.le.2*nmo+ncore) naomo = 2*nmo+ncore
                    call abmax(qq(ivoff+khaomo),naomo,nmo+1,'tri',iorb2,
     +                         bm,lm)
                  else
c                    lm = j-1
                     lm = nmo
...
c                     if (dabs(qq(ivoff+khaomo-1+ind(iorb2,lm))).lt.remcri
c     +                  .and.idequi.ne.3) goto 22
*
                     if (dabs(qq(ivoff+khaomo-1+ind(iorb2,lm))).lt.
     +                  remcri.and.idequi.ne.3) goto 22
                  end if
                  h2=dabs(qq(ivoff+khaomo-1+ind(iorb2,lm)))
...
cjvl                  end if
                  iex(iii,iorb2)=lm
                  qq(ivoff+khaomo-1+ind(iorb2,lm)) = 0.0d0
40             continue
            end if
30       continue
...
c
     ...
     &                        'kscr3',IGMEM_DEBUG)
      call mult11(qq(ivoff+khao),haomo,qq(ivoff+kscr2),nam,nao,vec,
     &            qq(ivoff+kscr3))
c
c...  as core orbitals may be anything 
c...  clear their interaction to avoid hybry trouble
...
c
     ...
     &                          'kscr',IGMEM_DEBUG)
         ...
         if (hybry) call clmetr(qq(ivoff+ksao),nbasis,'any')
c
         koccp = igmem_alloc_inf(ndim*nocc,'vbscf.m','virtual',
     &                           'koccp',IGMEM_DEBUG)
         call fmove(occ,qq(ivoff+koccp),nbasis*nocc)
c
c...  be sure to clear parts that were not cleared because of frozen
c...  or core ---
c
         if (clean.and.hybry) then
            call clvecvb(qq(ivoff+koccp),
...
c...  schmidt orthogonalise the occupieds
     ...
     &                                IGMEM_DEBUG)
            call normvc(qq(ivoff+koccp),qq(ivoff+ksao),
     &                  qq(ivoff+kscr),nbasis,nocc,cridep)
            call gmem_free_inf(kscr,'vbscf.m','virtual','kscr')
         end if
c
...
c
            call anorm(qq(ivoff+kscr),qq(ivoff+1))
            call tback(qq(ivoff+koccp),ilifq,qq(ivoff+koccp),ilifq,nocc)
c...     get vectors closely packed but in the 2-d-array
            call comharm(qq(ivoff+koccp),harmvb,ilifq)
c
c...   get s-matrix in reduced harmonic basis
c
            call comharm(flop,'ctrans',flop)
            call tranp(qq(ivoff+ksao),qq(ivoff+kscr))
            call dcopy(ns,qq(ivoff+kscr),1,qq(ivoff+ksao),1)
            call gmem_free_inf(kscr,'vbscf.m','virtual','kscr')
         else
...
c            print *,'remcolocm',loc( qq(ivoff+kprma+k-1)),
c.     &       loc(qq(ivoff+koccp-1+i+ilifq(n))),
c     &  loc(qq(ivoff+koccp-1+j+ilifq(n)))
c               call remcoi('prma again',k) 
                  qq(ivoff+kprma+k-1) = qq(ivoff+kprma+k-1) +
     &                  qq(ivoff+koccp-1+i+ilifq(n))
     &                 *qq(ivoff+koccp-1+j+ilifq(n))
               end do
            end do
         end do
c...  clear density matrix to conform to hybrid
c...  (*to be sure*  is *not* needed* ??? and confuses harmon
ccc      call clmetr(qq(ivoff+kprma),nbasis,'small')
c
         if (iprins.ge.50000) then
            write(iwr,*) ' projection operator'
            call tripri(qq(ivoff+kprma),newbas0)
         end if
c
...
c
     ...
     &                             IGMEM_DEBUG)
         call jaccov(qq(ivoff+kprma),newbas0,qq(ivoff+ksao),
     &               qq(ivoff+kvec),qq(ivoff+kval),3,crilow,
     &               qq(ivoff+kscr))
c
c...  harmonic
c
         if (oharm) then
            call expharm(qq(ivoff+kvec),'vectors',ilifq)
            call expharm(flop,'ctrans',flop)
c...     we just make a few zero virtuals (toetoe should sort them out)
            call tdown(qq(ivoff+kvec),ilifq,qq(ivoff+kvec),ilifq,nbasis)
...
c
         if (iprins.ge.50000) then
            ...
            call prvc(qq(ivoff+kvec),nbasis,nbasis,
     &                qq(ivoff+kval),'v','l')
         end if
c
         call gmem_free_inf(kscr,'vbscf.m','virtual','kscr')
...
c
         call fmove(qq(ivoff+kvir),virt,nvirt*nbasis)
c
...
c...      note nbasis may not be enough
           call vclr(virt,1,nbasis*nbasis)
           call vclr(qq(ivoff+ksh1),1,nbasis)
           call vclr(qq(ivoff+ksh2),1,nbasis)
c
...
9876          iatom = iatom+1
     ...
     &                         qq(ivoff+kvhyb),nopa(iatom),nbasis,
     &                         qq(ivoff+1))
...
c
                    do j=1,nopa(i)-igno_sh(i)
                       k = iopa(j,i)
                       if (qq(ivoff+ksh1+k-1).eq.0.0d0) then
                          qq(ivoff+ksh1+k-1) = 1.0d0
                          kk = kk + 1
                          virt(k,kk) = 1.0d0
                          qq(ivoff+ksh2+k-1) = kk
                       end if
                    end do
               ...
               do iatom=1,natom
                  do imo=1,nacat(iatom)
                     if (.not.ofrzmo(iacat(imo,iatom))) then
                        ...
                        do iao = 1,nex(nn)
                           kk = qq(ivoff+ksh2+(iopa(iao,iatom))-1)+0.1d0
...
c
       call vbcanon(occ,nbasis,nocc,canonicalise(1),'occ',qq(ivoff+1))
c
...
c
      ...
      real*8 occ(nbasis,*),virt(nbasis,*),vhyb(nopaa,nopaa)
      real*8 dummy
...
c
      ...
      do i=1,nopa(iatom)
         do j=1,i
            qq(ivoff+kshyb-1+itri(i,j)) = 
     1      qq(ivoff+ksao-1+itri(iopa(i,iatom),iopa(j,iatom)))
         end do
      end do
c
...
c....    scr = itri(n,n)+itri(n,n)+n*n+2*n*n (about)
      call jaccov(qq(ivoff+kprma),nopaa,qq(ivoff+kshyb),
     &            vhyb,qq(ivoff+kval),3,crilow,
     &            qq(ivoff+kscr))
c
      if (iprins.ge.50000) then
         write(iwr,*) ' eigenvectors/values'
         call prvc(vhyb,nopaa,nopaa,qq(ivoff+kval),'v','l')
      end if  
c
c.... expand
c
...
c...  common for basis change in VB
      real*8 corebas
      integer isecbas,nbasbas,ncolbas
      common/basisvb/corebas,isecbas,nbasbas,ncolbas,isbass
c
c     core :  frozen core energy for the whole job
c     core_i :  frozen core energy  in makeps0 
c
...
c
     ...
     &                         IGMEM_DEBUG)
c
      call fmove(v,qq(ivoff+kvec),ncol*nbasis)
c
...
c
      if (p0core.and.ndoub.gt.0) then
         call swapvec(v,qq(ivoff+kvec),qq(ivoff+kidoub),
...
c
      ...
      call transformvb(qq(ivoff+ks),qq(ivoff+kh),
     &                 qq(ivoff+kvec))
...
c
      ...
      call hsmat(qq(ivoff+ks),qq(ivoff+kh),qq(ivoff+kg),nsa,qq(ivoff+1))
      a1 = cpulft(1)
...
c...  DAVIDT SUBROUTINE STILL USING OLD MEMORY PARTITIONING
      call davidt(qq(ivoff+kscr),kmemscr,core,evb,'psi0')
c
c...  move ci-vector to right position
c
      do i=1,nstruc
         ci(i)=qq(ivoff+kscr+i-1)
      end do
...
c
      call sym1(qq(ivoff+ks),nsa,qq(ivoff+kiort),northo,'noprint')
c   remco
c...  read information from datatape
...
c
     ...
     &                        'kdete',IGMEM_DEBUG)
      ...
      call psi0det(ci,nstruc,qq(ivoff+kigro),ngroup,qq(ivoff+kcoef),
     &             ncoeff,ndets,qq(ivoff+kndet),qq(ivoff+kidps),
     &             qq(ivoff+kidet),qq(ivoff+kjdet),qq(ivoff+kpacd),
     &             nelec,nalfa,ndettot,qq(ivoff+kdete),
     &             kmemleft,'save') 
...
c
      call unpack(qq(ivoff+kpacd),n8_16,qq(ivoff+kidet),ndettot*nelec)
c
...
c...  qq(kg) => qq(kg+1)
c
      call natorbt(qq(ivoff+kdete),ndettot,qq(ivoff+kidet),
     &             qq(ivoff+kjdet),qq(ivoff+kjjdet),nelec,nalfa,
     &             qq(ivoff+kdao),qq(ivoff+kdmo),qq(ivoff+kipos),
     &             qq(ivoff+kvnat),qq(ivoff+kiort),northo,nbasis,nsa,
     &             ncore,qq(ivoff+kh),qq(ivoff+ks),qq(ivoff+kg+1))
...
c
      ...
      real*8 vin(nbasis*nbasis),vout(nbasis*nbasis)
...
************************************************************************
      ...
      real*8 vnew(nbasis,norb),vold(nbasis,norb)
      real*8 smo(*),h1(*),h2(*),hp(*)
      real*8 smax,signm,ddot
...
*
     ...
     +                                IGMEM_DEBUG)
*
      call normvc(v,qq(ivoff+ksao),qq(ivoff+kscr),nbasis,nocc,cridep)
      call gmem_free_inf(kscr,'vbscf.m','orthovb','kscr')
