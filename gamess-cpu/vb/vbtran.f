...
c  last(??) fix for i8 VB; packed arrays are real*8 now; however i*2 and i*4 is used
...
c
     ...
     &                          IGMEM_DEBUG)
      iflop = lchbas(vect,qq(ivoff+kscr),ncol,'external')
...
cremco   call prvc(vect,ncol,nbasis,vect,'o','l')
      end if
c
...
c...  copy orbitals
c
      call vclr(qq(ivoff+kvec),1,ncol*nbasis)
      call vclr(qq(ivoff+ktvec),1,nsa*nbasis)
      call dcopy(ncol*nbasis,vect,1,qq(ivoff+kvec),1)
...
c...     get ao metric
c
         call get1e(qq(ivoff+ksao),dummy,'s',qq(ivoff+ksao))
c
c...     schmidt core orbitals
c
         call normvc(qq(ivoff+kvec),qq(ivoff+ksao),qq(ivoff+kscr),
     &               nbasis,ncore,cridep)
c
c...     schmidt other orbitals onto core orbitals
c
         call schmids(qq(ivoff+kvec+ncore*nbasis),qq(ivoff+kvec),
     &                qq(ivoff+ksao),qq(ivoff+kscr),
...
c
      ...
      call wrt3(qq(ivoff+kvec),(ncore+nsa)*nbasis,kvb7_transvb,num8)
c
c...  transpose active orbitals
c
      call dagger(nbasis,nsa,qq(ivoff+kvec+ncore*nbasis),
     &            nbasis,qq(ivoff+ktvec),nsa)
c
...
c
     ...
     &                           'kscr',IGMEM_DEBUG)
      call ind4vb(qq(ivoff+kvec+ncore*nbasis),
     &            qq(ivoff+ktvec),qq(ivoff+kscr),kmaxmem)
c    &            qq(ivoff+ktvec),qq(ivoff+kscr),kmemscr)
      call gmem_free_inf(kscr,'vbtran.m','transformvb','kscr')
c
...
c	call gmem_check_guards('voor shcbld')
      ...
      call shcbld(qq(ivoff+ksao),h,qq(ivoff+kvec),
     &            qq(ivoff+ktvec),qq(ivoff+kx2),
     &            qq(ivoff+kx3),qq(ivoff+kx1))
      call dcopy(lenact,qq(ivoff+ksao),1,s,1)
...
c
      if (iprint.ge.20000) then
         ...
         call prvc(qq(ivoff+kvec),ncol,nbasis,qq(ivoff+kvec),'o','l')
      end if
c
      kmaxmem = igmem_max_memory()
      kmemscr = kmaxmem/10
...
c      save ikyiky,nocc,nocc1,lbase
      ...
      integer iky(maxorb)
cc!$acc declare create(ikyiky,iky,nocc,nocc1,lbase)
      end module
...
c
      if (incsort) then
         call fmove(qq(ivoff+ksort+nsz512*(iblock-1)),gin,nsz512)
      else
...
c
      if (incsort) then
         call fmove(gout,qq(ivoff+ksort+nsz512*(iblock-1)),nsz512)
