...
c
      implicit real*8 (a-h,o-z), integer (i-n)
c
      real*8 grh(*),grs(*),diagh(*),diags(*)
      real*8 detcomb(*), dettot(*)
      integer igroup(5,*)
      real*8 pacdet(*) ! Should be pacdet(ngroup) but I cannot tell how
                     ! many words unpack might access. So I had to be
                     ! conservative in the declaration to avoid copy-in
                     ! copy-out problems. HvD.
      real*8 coeff(*)
      real*8 supers(*),superh(*),superg(*)
      real*8 scr1(*),scr2(*),scr3(*)
      real*8 s(*),g(*),weight(*)
      integer idet(nelec,*)
...
c.....
     ...
     &                                  supers,ipos,dprod,
     &                                  nbody,ig,s)
c             call sillyy(nelec,nalfa,ig,nbody,icp,jcp,qq(ivoff+iscopy))
               if (nsing.le.2) then
