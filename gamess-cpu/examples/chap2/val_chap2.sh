...
export GAMESS_VTAB=./chap2.vtab

# Set list of jobs
job_list=`cat $chap2dir/jobs.list | grep -v "#"`
...
# Print header
echo 
echo "The results of the chapter 2 examples"
echo "====================================="
echo 
...
for job in $job_list
  ...
  if ( test -e $chap2dir/LOG/${job}.log )
	  ...
	  # Using old vtab stuff
          # 
	  ../../utilities/validate $job $chap2dir/LOG/${job}.log || error=1
      else
          #
	  ...
	  if ( test \( ${status} -eq 1 \) )
		  ...
		  then
		  tkdiff ${chap2dir}/REF/${job}.log ${chap2dir}/LOG/${job}.log
	      elif ( test  ${text} -eq 1 )
		  then
		  $toldiff --output full ${chap2dir}/REF/${job}.log ${chap2dir}/LOG/${job}.log
	      fi
	  fi
      fi
  else
