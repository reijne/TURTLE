...
toldiffdir=$nbodir/../../utilities/toldiff
...
# Set list of jobs
job_list=`cat $nbodir/jobs.list | grep -v "#"`
...
# Print header
echo 
echo "The results of the NBO chapter examples"
echo "======================================="
echo 

for job in $job_list
  do
  if ( test -e $nbodir/LOG/${job}.log )
      then
      if ( test $oldval -eq 1 )
	  then
          #
	  # Using old vtab stuff
          # 
	  ../../utilities/validate $job $nbodir/LOG/${job}.log || error=1
      else
          #
	  # New toldiff approach
          #
	  result=`$toldiff $nbodir/REF/${job}.log $nbodir/LOG/${job}.log`
	  status=$?
	  printf "%30s %8s\n" ${job} ${result}
	  if ( test \( ${status} -eq 1 \) )
	      then
		  ...
		  then
		  tkdiff ${nbodir}/REF/${job}.log ${nbodir}/LOG/${job}.log
	      elif ( test  ${text} -eq 1 )
		  then
		  $toldiff --output full ${nbodir}/REF/${job}.log ${nbodir}/LOG/${job}.log
	      fi
	  fi
      fi
  else
