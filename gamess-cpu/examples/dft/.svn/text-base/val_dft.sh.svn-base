...
toldiffdir=$dftdir/../../utilities/toldiff
...
# Set list of jobs
job_list=`cat $dftdir/jobs.list | grep -v "#"`
...
# Print header
echo 
echo "The results of the DFT chapter examples"
echo "======================================="
echo 

for job in $job_list
  do
  if ( test -e $dftdir/LOG/${job}.log )
      then
      if ( test $oldval -eq 1 )
	  then
          #
	  # Using old vtab stuff
          # 
	  ../../utilities/validate $job $dftdir/LOG/${job}.log || error=1
      else
          #
	  # New toldiff approach
          #
	  result=`$toldiff $dftdir/REF/${job}.log $dftdir/LOG/${job}.log`
	  status=$?
	  printf "%30s %8s\n" ${job} ${result}
	  if ( test \( ${status} -eq 1 \) )
	      then
		  ...
		  then
		  tkdiff ${dftdir}/REF/${job}.log ${dftdir}/LOG/${job}.log
	      elif ( test  ${text} -eq 1 )
		  then
		  $toldiff --output full ${dftdir}/REF/${job}.log ${dftdir}/LOG/${job}.log
	      fi
	  fi
      fi
  else
