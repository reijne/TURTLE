...
toldiffdir=$chap4dir/../../utilities/toldiff
...
# Set list of jobs
job_list=`cat $chap4dir/jobs.list | grep -v "#"`
...
# Print header
echo 
echo "The results of the chapter 4 examples"
echo "====================================="
echo 

for job in $job_list
  do
  if ( test -e $chap4dir/LOG/${job}.log )
      then
      if ( test $oldval -eq 1 )
	  then
          #
	  # Using old vtab stuff
          # 
	  ../../utilities/validate $job $chap4dir/LOG/${job}.log || error=1
      else
          #
	  # New toldiff approach
          #
	  result=`$toldiff $chap4dir/REF/${job}.log $chap4dir/LOG/${job}.log`
	  status=$?
	  printf "%30s %8s\n" ${job} ${result}
	  if ( test \( ${status} -eq 1 \) )
	      then
		  ...
		  then
		  tkdiff ${chap4dir}/REF/${job}.log ${chap4dir}/LOG/${job}.log
	      elif ( test  ${text} -eq 1 )
		  then
		  $toldiff --output full ${chap4dir}/REF/${job}.log ${chap4dir}/LOG/${job}.log
	      fi
	  fi
      fi
  else
