...
INT BitSet::count(INT b) const
{
  ...
  if (b == 1)
  {
    ...
    {
      _BS_word a = *s++;
      for (unsigned INT i = 0; i < BITSETBITS && a != 0; ++i)
      {
        if (a & 1)
          ++l;
        a >>= 1;
  ...
  else
  {
    ...
    {
      _BS_word a = *s++;
      for (unsigned INT i = 0; i < BITSETBITS; ++i)
      {
        if ((a & maxbit) == 0)
          ++l;
        a <<= 1;
...
void BitSet::set(INT p)
{
  ...
  unsigned INT index = BitSet_index(p);
...
INT BitSet::next(INT p, INT b) const
{
  ...
  else
  {
    ...
    for (++j; j < l; ++j)
    {
      ...
      if (a != ONES)
      {
        for (unsigned i = 0; i < BITSETBITS; ++i)
        {
          if ((a & 1) == 0)
            return j * BITSETBITS + i;
          a >>= 1;
...
const char* BitSettoa(const BitSet& x, char f, char t, char star)
{
  ...
  ostringstream stream(fmtbase, ostringstream::in | ostringstream::out);
...
BitSet longtoBitSet(unsigned LONG_INT i)
{
  BitSet r;
#if 1
  _BS_word w = i;
  ...
  _BS_word u[2];
  u[0] = i & ((_BS_word)(~(0)));
  u[1] = sizeof(LONG_INT) <= sizeof(_BS_word) ? 0 : i >> BITSETBITS;
...
// FIXME:  Does not respect s.width()!
{
  ...
  {
    _BS_word a = *s++;
    for (unsigned INT j = 0; j < BITSETBITS; ++j)
    {
      sb->sputc((a & 1)? t : f);
      a >>= 1;
    }
  ...
  if (!rep->virt)
  {
    ...
    if (rep->len != 0)
    {
      for (unsigned INT j = 0; j < BITSETBITS && a != 0; ++j)
      {
        sb->sputc((a & 1)? t : f);
        a >>= 1;
      }
  ...
  else
  {
    ...
    if (rep->len != 0)
    {
      for (unsigned INT j = 0; j < BITSETBITS && a != mask; ++j)
      {
        sb->sputc((a & 1)? t : f);
        a = (a >> 1) & himask;
        mask = (mask >> 1) & himask;
