...
{
  INT x;
  unsigned LONG_INT hash = (unsigned LONG_INT)rx_bitset_hash;
...
static unsigned LONG_INT rx_hash_masks[4] =
{
  0x12488421,
  0x96699669,
  0xbe7dd7eb,
...
#endif
{
  ...
  
  {
	...
	newtab->parent = table;
	{
	  ...
	  unsigned LONG_INT newmask = rx_hash_masks[maskc + 1];
	  while (them)
	    {
  ...
  if (it)
    {
      ...
      unsigned LONG_INT hash = it->hash;
...
#endif
{
  LONG_INT hash = (LONG_INT)car ^ (LONG_INT)cdr;
...
rx_compactify_nfa (struct rx *rx,
		   void **mem, unsigned LONG_INT *size)
#else
RX_DECL INT 
rx_compactify_nfa (rx, mem, size)
     struct rx *rx;
     void **mem;
     unsigned LONG_INT *size;
#endif
{
  INT total_nodec;
  struct rx_nfa_state *n;
	...
	while (eclose)
	  {
		...
		sp = rx_hash_find (&rx->se_list_memo,
				   (LONG_INT)sesrc->car ^ (LONG_INT)sesrc->cdr,
	    ...
	    *sepos = sesrc;
		...
		sp = rx_hash_find (&rx->set_list_memo,
				   ((((LONG_INT)destlst->car) >> 8)
				    ^ (LONG_INT)destlst->cdr),
  ...
  if (!--set->refs)
	...
	(rx_hash_find
	 (&cache->superset_table,
	  (unsigned LONG_INT)set->car ^ set->id ^ (unsigned LONG_INT)set->cdr,
	  (void *)set,
...
#endif
{
  struct rx_cache * cache
    = ((struct rx_cache *)
       ((char *)rules
	- (unsigned LONG_INT)(&((struct rx_cache *)0)->superset_hash_rules)));
...
#endif
{
  struct rx_cache * cache
    = ((struct rx_cache *)
       (char *)rules - (LONG_INT)(&((struct rx_cache *)0)->superset_hash_rules));
...
#endif
{
    ...
    }
  {
    ...
    hit = rx_hash_store (&cache->superset_table,
			 (unsigned LONG_INT)car ^ car->id ^ (unsigned LONG_INT)cdr,
			 (void *)&template,
			 &cache->superset_hash_rules);
    return (hit
...
{
  0,						/* REG_NOUT */
  "No match",					/* REG_NOMATCH_RX */
  "Invalid regular expression",			/* REG_BADPAT_RX */
  "Invalid collation character",		/* REG_ECOLLATE_RX */
  "Invalid character class name",		/* REG_ECTYPE_RX */
  "Trailing backslash",				/* REG_EESCAPE_RX */
  "Invalid back reference",			/* REG_ESUBREG_RX */
  "Unmatched [ or [^",				/* REG_EBRACK_RX */
  "Unmatched ( or \\(",				/* REG_EPAREN_RX */
  "Unmatched \\{",				/* REG_EBRACE_RX */
  "Invalid content of \\{\\}",			/* REG_BADBR_RX */
  "Invalid range end",				/* REG_ERANGE_RX */
  "Memory exhausted",				/* REG_ESPACE_RX */
  "Invalid preceding regular expression",	/* REG_BADRPT_RX */
  "Premature end of regular expression",	/* REG_EEND_RX */
  "Regular expression too big",			/* REG_ESIZE_RX */
  "Unmatched ) or \\)",				/* REG_ERPAREN_RX */
};
 ...
 */
#define PATFETCH(c)							\
 do {if (p == pend) return REG_EEND_RX;					\
    c = (unsigned char) *p++;						\
    c = translate[c];		 					\
 } while (0)
...
#endif
{
  ...
  if (range_start > range_end)
    return syntax & RE_NO_EMPTY_RANGES ? REG_ERANGE_RX : REG_NOERROR_RX;
...
#endif
{
  if (rexp)
	...
	return;
      case r_side_effect:
	if (   ((LONG_INT)rexp->params.side_effect >= 0)
	    && (params [(LONG_INT)rexp->params.side_effect].se == re_se_backref))
	  out[ params [(LONG_INT)rexp->params.side_effect].op1] = 1;
	return;
      }
}
...
#endif
{
  ...
  if (!rexp)
	...
	if (l)
      ...
      case r_side_effect:
	{
	  INT se = (LONG_INT)rexp->params.side_effect;
	  if (   (se >= 0)
...
#endif
{
  ...
  switch (node->type)
    ...
    case r_star:
      return pointless_if_repeated (node->params.pair.left, params);
    case r_side_effect:
      switch (((LONG_INT)node->params.side_effect < 0)
	      ? (enum re_side_effects)node->params.side_effect
	      : (enum re_side_effects)params[(LONG_INT)node->params.side_effect].se)
	{
	case re_se_try:
	case re_se_at_dot:
	case re_se_begbuf:
...
#endif
{
  ...
  switch (rexp->type)
    ...
    case r_side_effect:
      return
	!((LONG_INT)rexp->params.side_effect > 0
	  ? idempotent_complex_se [ params [(LONG_INT)rexp->params.side_effect].se ]
	  : idempotent_se [-(LONG_INT)rexp->params.side_effect]);
...
#endif
{
  ...
  switch (rexp->type)
    ...
    case r_side_effect:
      return ((LONG_INT)rexp->params.side_effect >= 0);
            ...
            for (;;)
              {
                ...
                if (c == '*'
                  ...
                  {
                    if (p == pend) return REG_EESCAPE_RX;
	...
	case '.':
	  {
        ...
        case '[':
	  if (p == pend) return REG_EBRACK_RX;
          {
		      ...
		      rx_bitset_union (rxb->rx.local_cset_size, cs, it);
                ...
                if (c == '-' 
                    ...
                    && *p != ']')
                  {
                    ...
                    if (ret != REG_NOERROR_RX) return ret;
                  }
                    ...
                    if (c == ':' && *p == ']')
                      {
                        ...
                        if (!IS_CHAR_CLASS (str)) return REG_ECTYPE_RX;
        ...
        case '\\':
          ...
          switch (c)
		   ...
		   (compile_stack_elt_t *) realloc (compile_stack.stack, ( compile_stack.size << 1) * sizeof (
													      compile_stack_elt_t)));
                  if (compile_stack.stack == 0) return REG_ESPACE_RX;
 ...
 */
...
#endif
  {
	...
	unsigned LONG_INT size = rxb->allocated;
	INT start_id;
	char * perm_mem;
     ...
     INT stop;
{
  ...
  if (pos->pos == pos->end)
    {
      if (((void*)pos->string == (void*) closure->string1)
	  && (closure->string2)
	  && (closure->size2 > 1))
...
/* Set REGS to hold NUM_REGS registers, storing them in STARTS and
   ...
   be at least NUM_REGS * sizeof (regoff_t_rx) bytes long.
...
re_set_registers (struct re_pattern_buffer *bufp,
		  struct re_registers *regs,
		  unsigned num_regs,
		  regoff_t_rx * starts, regoff_t_rx * ends)
#else
void
...
#endif
{
  if (num_regs)
  ...
  else
    {
      ...
      regs->num_regs = 0;
      regs->start = regs->end = (regoff_t_rx) 0;
    }
}
...
/* For now we leave these out, because regex_t_rx is not binary
...
/* regcomp_rx takes a regular expression as a string and compiles it.

   PREG is a regex_t_rx *.  We do not expect any fields to be initialized,
   since POSIX says we shouldn't.  Thus, we set

     `buffer' to the compiled pattern;
     `used' to the length of the compiled pattern;
     `syntax' to RE_SYNTAX_POSIX_EXTENDED if the
       REG_EXTENDED_RX bit in CFLAGS is set; otherwise, to
       RE_SYNTAX_POSIX_BASIC;
     `newline_anchor' to REG_NEWLINE_RX being set in CFLAGS;
   ...
   CFLAGS is a series of bits which affect compilation.
     ...
     If REG_EXTENDED_RX is set, we use POSIX extended syntax; otherwise, we
     use POSIX basic syntax.

     If REG_NEWLINE_RX is set, then . and [^...] don't match newline.
     Also, regexec_rx will try a match beginning after every newline.

     If REG_ICASE_RX is set, then we considers upper- and lowercase
     versions of letters to be equivalent when matching.

     If REG_NOSUB_RX is set, then when PREG is passed to regexec_rx, that
...
#ifdef __STDC__
INT
regcomp_rx (regex_t_rx * preg, __const__ char * pattern, INT cflags)
#else
INT
regcomp_rx (preg, pattern, cflags)
    regex_t_rx * preg;
    __const__ char * pattern;
    INT cflags;
#endif
{
  reg_errcode_t ret;
  unsigned syntax
    = cflags & REG_EXTENDED_RX ? RE_SYNTAX_POSIX_EXTENDED : RE_SYNTAX_POSIX_BASIC;
  ...
  if (cflags & REG_ICASE_RX)
    {
      unsigned i;

      preg->translate = (unsigned char *) malloc (256);
      if (!preg->translate)
        return (INT) REG_ESPACE_RX;
  ...
  /* If REG_NEWLINE_RX is set, newlines are treated differently.  */
  if (cflags & REG_NEWLINE_RX)
    { /* REG_NEWLINE_RX implies neither . nor [^...] match newline.  */
  ...
  preg->no_sub = !!(cflags & REG_NOSUB_RX);
  ...
  /* POSIX doesn't distinguish between an unmatched open-group and an
     unmatched close-group: both are REG_EPAREN_RX.  */
  if (ret == REG_ERPAREN_RX) ret = REG_EPAREN_RX;

  return (INT) ret;
}


/* regexec_rx searches for a given pattern, specified by PREG, in the
   string STRING.

   If NMATCH is zero or REG_NOSUB_RX was set in the cflags argument to
   `regcomp_rx', we ignore PMATCH.  Otherwise, we assume PMATCH has at
   least NMATCH elements, and we set them to the offsets of the
   corresponding matched substrings.

   EFLAGS specifies `execution flags' which affect matching: if
   REG_NOTBOL_RX is set, then ^ does not match at the beginning of the
   string; if REG_NOTEOL_RX is set, then $ does not match at the end.

   We return 0 if we find a match and REG_NOMATCH_RX if not.  */

#ifdef __STDC__
INT
regexec_rx (__const__ regex_t_rx *preg, __const__ char *string,
	 size_t nmatch, regmatch_t_rx pmatch[],
	 INT eflags)
#else
INT
regexec_rx (preg, string, nmatch, pmatch, eflags)
    __const__ regex_t_rx *preg;
    __const__ char *string;
    size_t nmatch;
    regmatch_t_rx pmatch[];
    INT eflags;
#endif
{
  INT ret;
  struct re_registers regs;
  regex_t_rx private_preg;
  INT len = strlen (string);
  boolean want_reg_info = !preg->no_sub && nmatch > 0;

  private_preg = *preg;

  private_preg.not_bol = !!(eflags & REG_NOTBOL_RX);
  private_preg.not_eol = !!(eflags & REG_NOTEOL_RX);
  ...
  if (want_reg_info)
    {
      ...
      regs.start =  (( regoff_t_rx *) malloc ((nmatch) * sizeof ( regoff_t_rx)));
      regs.end =  (( regoff_t_rx *) malloc ((nmatch) * sizeof ( regoff_t_rx)));
      if (regs.start == 0 || regs.end == 0)
        return (INT) REG_NOMATCH_RX;
    }
  ...
  return ret >= 0 ? (INT) REG_NOERROR_RX : (INT) REG_NOMATCH_RX;
}


/* Returns a message corresponding to an error code, ERRCODE, returned
   from either regcomp_rx or regexec_rx.   */

#ifdef __STDC__
size_t
regerror_rx (INT errcode, __const__ regex_t_rx *preg,
	  char *errbuf, size_t errbuf_size)
#else
size_t
regerror_rx (errcode, preg, errbuf, errbuf_size)
    INT errcode;
    __const__ regex_t_rx *preg;
    char *errbuf;
    size_t errbuf_size;
