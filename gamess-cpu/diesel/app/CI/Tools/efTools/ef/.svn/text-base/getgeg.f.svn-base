...
************************************************************************        
   ...
   40    LABELS(NATOMS)=J                                                       
         IF(J.NE.99)THEN                                                        
            NUMAT=NUMAT+1                                                       
C           ATMASS(NUMAT)=READA(LINE(1:MAX(ISTART(2)-1,1)),ISTART(1))           
            IADEX(NUMAT)=
     &            IDINT(READA(LINE(1:MAX(ISTART(2)-1,1)),ISTART(1)))           
C           IF(ATMASS(NUMAT).GT.1.D-15)THEN                                     
C              WRITE(6,'('' FOR ATOM'',I4,''  ISOTOPIC MASS:''                  
C    1    ,F15.5)')NATOMS, ATMASS(NUMAT)                                        
C              ATMASS(NUMAT)=AMS(J)                                             
C           ELSE                                                                
               ATMASS(NUMAT)=AMS(J)                                             
C           ENDIF                                                               
