...
c
      real*8 kma   ,kmb                       
      real*8 grad                       
      real*8 ba_ao                         ! the alpha b vector (Au=b)
      real*8 bb_ao                         ! the beta  b vector (Au=b)
      real*8 ba_mo                         ! the alpha b vector (Au=b)
      real*8 bb_mo                         ! the beta  b vector (Au=b)
      real*8 ga_ao                         ! the alpha Au matrix (Au=b)
      real*8 gb_ao                         ! the beta  Au matrix (Au=b)
      real*8 ga_mo                         ! the alpha Au matrix (Au=b)
      real*8 gb_mo                         ! the beta  Au matrix (Au=b)
      real*8 fa_ao                         ! the alpha perturbed fock
                                         ! matrix
      real*8 fb_ao                         ! the beta  perturbed fock
                                         ! matrix
      real*8 fa_mo                       
      real*8 fb_mo                       
      real*8 fxa_ao                        ! explicit derivative alpha
                                         ! fock matrix
      real*8 fxb_ao                        ! explicit derivative beta
                                         ! fock matrix
      real*8 fxa_mo                       
      real*8 fxb_mo                       
      real*8 hess             
...
C Local variables
      ...
      integer na_mo, nb_mo
      real*8 rpt,rwt
      real*8 atom_xce(max_atom),atom_den(max_atom,2)
      integer atom_pts(max_atom)
      real*8 xc_e
      real*8 del_psi, del_rho
      real*8 rshell, rnear
      integer next, ltri
      real*8 fac
      integer mxp
...
c     
      real*8 t_now
      real*8 t_start, t_then
      real*8 t_becke, t_bas, t_den, t_pack
      real*8 t_geom, t_xc, t_ks, t_grad
      LOGICAL time_cond
...
c
      ...
      logical geometric_pert_sw ! true if geometric perturbations are
                                ! considered, otherwise the source of
                                ! the perturbation is an external field.
      real*8 avec(nao,nvec)       ! the alpha MOs
      real*8 bvec(nao,nvec)       ! the beta  MOs
      real*8 adens(nao*(nao+1)/2) ! the alpha density matrix in AO-basis
      real*8 bdens(nao*(nao+1)/2) ! the beta  density matrix in AO-basis
      real*8 sa_ao(nao,nao,npert) ! the derivative of the occupied-
                                ! occupied block of the overlap matrix
      real*8 sb_ao(nao,nao,npert) ! the derivative of the occupied-
                                ! occupied block of the overlap matrix
      real*8 sa_mo(nvec*(nvec+1)/2,npert)
      real*8 sb_mo(nvec*(nvec+1)/2,npert)
      real*8 ua_ao(nao,nao,npert) ! the alpha u matrix (Au=b)
      real*8 ub_ao(nao,nao,npert) ! the beta  u matrix (Au=b)
      real*8 ua_mo(naocc,nvec-naocc,npert) ! the alpha u matrix (Au=b)
      real*8 ub_mo(nbocc,nvec-nbocc,npert) ! the beta  u matrix (Au=b)
      real*8 da_ao(nao,nao,npert) ! the alpha perturbed density matrix 
      real*8 db_ao(nao,nao,npert) ! the beta  perturbed density matrix 
...
c
      real*8 kma(*),kmb(*)
      real*8 grad(3,*)
      real*8 ba_ao(nao,nao,npert)          ! the alpha b vector (Au=b)
      real*8 bb_ao(nao,nao,npert)          ! the beta  b vector (Au=b)
      real*8 ba_mo(naocc,nvec-naocc,npert) ! the alpha b vector (Au=b)
      real*8 bb_mo(nbocc,nvec-nbocc,npert) ! the beta  b vector (Au=b)
      real*8 ga_ao(nao,nao,npert)          ! the alpha Au matrix (Au=b)
      real*8 gb_ao(nao,nao,npert)          ! the beta  Au matrix (Au=b)
      real*8 ga_mo(naocc,nvec-naocc,npert) ! the alpha Au matrix (Au=b)
      real*8 gb_mo(nbocc,nvec-nbocc,npert) ! the beta  Au matrix (Au=b)
      real*8 fa_ao(nao,nao,npert)          ! the alpha perturbed fock 
                                         ! matrix 
      real*8 fb_ao(nao,nao,npert)          ! the beta  perturbed fock 
                                         ! matrix 
      real*8 fa_mo(nvec*(nvec+1)/2,npert) 
      real*8 fb_mo(nvec*(nvec+1)/2,npert)
      real*8 fxa_ao(nao*(nao+1)/2,npert)   ! explicit derivative alpha
                                         ! fock matrix
      real*8 fxb_ao(nao*(nao+1)/2,npert)   ! explicit derivative beta
                                         ! fock matrix
...
C  Work arrays
      ...
      real*8 prpt(ngtypes,*)
      real*8 prwt(ngtypes,*)
      real*8 ra2_val(mxp0*natoms*2)
      real*8 ra2_comp(mxp0*natoms*3)
      real*8 rho(mxp0*2),grho(mxp0*2*3)
      integer bfn_radii_pt
      integer nprt
c
c still to be allocated
...
C Local variables
      ...
      integer atom_pts(max_atom)
      real*8 xc_e
      real*8 del_psi, del_rho, del_wgh
      real*8 rshell, rnear
      integer next, ltri
      real*8 fac
      integer npts
...
c
      ...
      real*8 accuracy_warn_tol
      real*8 accuracy_fatal_tol
...
c     Inputs/Outputs:
c
      real*8 kma(nao*(nao+1)/2)
      real*8 kmb(nao*(nao+1)/2)
      ...
      real*8 fa_mo(nvec*(nvec+1)/2,npert)
      real*8 fb_mo(nvec*(nvec+1)/2,npert)
      real*8 fxa_ao(nao*(nao+1)/2,npert)
      real*8 fxb_ao(nao*(nao+1)/2,npert)
      real*8 fxa_mo(nvec*(nvec+1)/2,npert)
      real*8 fxb_mo(nvec*(nvec+1)/2,npert)
      real*8 hess(npert,npert)
c
...
c
       integer nang
       real*8 p(3,nang),w(nang)
c
c      Outputs
c
       integer iperm(nang)
       real*8 po(3,nang),wo(nang)
c
c      Local variables
c
       integer npoints(4)
...
c        values.
c
      real*8 srad, sg1rad, mkrad, lograd
...
c
      ...
      real*8 rpt     ! the radius of the current shell
      real*8 rwt     ! the weight of the radial point
      real*8 apts(3,1202)
      real*8 atom_c(max_atom,3)
      real*8 awpt(1202)
c
c     Output:
c
      real*8 ra2_comp(mxp,natm,3) ! difference between point and atom
      real*8 ra2_val(mxp,natm,2)  ! distance between point and atom
      real*8 wt(mxp)              ! the weight of the point
c
c     Local:
c
...
c
      ...
      real*8    arad(natoms)         ! the radii of the atoms
...
c
      ...
      real*8    brad(nao)            ! the radii of the basis functions
c
c     Output:
c
...
C *Shell order information include file
     ...
     &     bf_start(maxL),bf_end(maxL),bf_num(maxL)
      ...
      real*8 ra2_comp(mxp,natoms,3)
      real*8 ra2_val(mxp,natoms,2)
      integer tag, nao
      integer ider, iout
c
C     *Out variables
...
c      
      ...
      real*8 tol
      real*8 xd(500),yd(500),zd(500),ra2(500),ra1(500)
      ...
      real*8 alp,cc,expo,minalp,minexp,alpexp
      integer px,py,pz,dxx,dyy,dzz,dxy,dxz,dyz
      integer ipt

      real*8 g,dg,ddg,dgu,ddgu
      real*8 gx,gy,gz
      real*8 gxr,gyr,gzr
      real*8 dgx,dgy,dgz
      real*8 dgxx,dgyy,dgzz,dgxy,dgxz,dgyz
      real*8 dgxxr,dgyyr,dgzzr,dgxyr,dgxzr,dgyzr
      real*8 dgxxx,dgxxy,dgxxz,dgxyy,dgyyy
      real*8 dgyyz,dgxzz,dgyzz,dgzzz,dgxyz
      real*8 ddgx,ddgy,ddgz
      real*8 ddgxxx,ddgxxy,ddgxxz,ddgxyy,ddgxyz,ddgxzz,ddgyyy
      real*8 ddgyyz,ddgyzz,ddgzzz
      real*8 ddgxxxx,ddgxxxy,ddgxxxz,ddgxxyy,ddgxxyz,ddgxxzz
      real*8 ddgxyyy,ddgxyyz,ddgxyzz,ddgxzzz,ddgyyyy,ddgyyyz
      real*8 ddgyyzz,ddgyzzz,ddgzzzz

      real*8 g3
      real*8 alp2, alp4, alp6, alp8, alp10, alp12, alp24, alp36
      real*8 x0d1, x0d2, x0d3
      real*8 x1d0, x1d1, x1d2, x1d3
      real*8 x2d0, x2d1, x2d2, x2d3
      real*8 y0d1, y0d2, y0d3
      real*8 y1d0, y1d1, y1d2, y1d3
      real*8 y2d0, y2d1, y2d2, y2d3
      real*8 z0d1, z0d2, z0d3
      real*8 z1d0, z1d1, z1d2, z1d3
      real*8 z2d0, z2d1, z2d2, z2d3
...
c
      ...
      real*8 avec(nao,navec) ! the alpha-vectors
      real*8 bvec(nao,nbvec) ! the beta-vectors
      real*8 bfn_val(mxp,nao)
      real*8 bfng_val(mxp,nao,3)
      real*8 bfn_hess(mxp,nao,6)
      integer ider
...
c     Outputs:
c
      real*8 amo_val(npts,navec)
      real*8 amo_grad(npts,navec,3)
      real*8 amo_hess(npts,navec,6)
      real*8 bmo_val(npts,nbvec)
      real*8 bmo_grad(npts,nbvec,3)
      real*8 bmo_hess(npts,nbvec,6)
c
c     Local variables
c
      integer ivec
...
c
      ...
      integer navec   ! the number of alpha-vectors
      integer nbvec   ! the number of beta-vectors
      real*8 avec(nao,nvec) ! the alpha-vectors
      real*8 bvec(nao,nvec) ! the beta-vectors
      real*8 bfn_val(mxp,nao)
      real*8 bfng_val(mxp,nao,3)
      real*8 bfn_hess(mxp,nao,6)
      integer ider
...
C In variables	
      ...
      real*8 adens(nao*(nao+1)/2),bdens(nao*(nao+1)/2)
      real*8 rho(mxp,2)
      real*8 bfng_val(mxp,nao,3)
...
c local
      ...
      real*8 r, rtest, delr, rmin
      real*8 gg, cc, expo, alp, s, p, d, f, g
...
C *Local variables
      ...
      real*8 pfact(2,maxgleg),tarr1(maxgleg2),tarr2(maxgleg2)
      real*8 cosphi(maxgleg), sinphi(maxgleg)
      real*8 tol,thet,x,p,p1,dp,dp1,q,dq,d,dphi,pw,dxy,phi
...
C *Local variables                                                    
      integer lrpt, iat
      real*8 rpt, n2, ln2, n2m, i2p1m1, i2p1m, m
      real*8 x, r, dr
c
      r(x,m)=-log(1.0d0-x**m)
...
c
     ...
     +       natomic
      ...
      real*8 function mkrad(natomic,iout,extwr_sw)
c **********************************************************************
c *   Description:                                                     *
...
c        Ac-???
     ...
     +                'used for z=',natomic
      ...
      real*8 function lograd(natomic,iout,extwr_sw)
C **********************************************************************
C *   Description:				Version: 1.0           *
...
c
      ...
      real*8 xc_dtpt(mxp,2) 
      real*8 grho(mxp,2,3)
c
c     Input/Output:
c
      real*8 kma(nao*(nao+1)/2), kmb(nao*(nao+1)/2)
c
c     Output:
c
      real*8 sma(nao*(nao+1)/2)
c
c     Workspace:
c
      real*8 om_scr(nao), om_scr1(mxp,3)
c
c     Local:
c
      integer i,j,n
...
c
      ...
      integer n_active_bfn, active_bfn_list(n_active_bfn)
      real*8 bfn_val(mxp,n_active_bfn)
      real*8 bfng_val(mxp,nao,3)
      real*8 wt(npts)
      real*8 xc_vpt(mxp,2)
      real*8 xc_dvpt(mxp,3) 
...
c     Workspaces:
c
      real*8 om_scr(mxp,n_active_bfn), om_scr1(mxp,3), T(mxp)
c
c     Local:
c
      integer i,j,n,iki
...
cINCLUDE(common/dft_module_comm)
      ...
      real*8 bfng_val(mxp,nao,3)
      real*8 bfn_hess(mxp,nao,6)
      real*8 wt(mxp),gwt(3,mxp,num_near_atoms)
      real*8 xc_ept(mxp),xc_vpt(mxp,2),xc_dvpt(mxp,3),xc_dtpt(mxp,2)
      real*8 grho(mxp,2,3)
      integer n_active_bfn
      integer active_bfn_list(n_active_bfn)
      integer n_active_bfn_atm
...
c     Workspace:
c
      real*8 scr2(3,nao),scr3(nao),scr4(npts,3)
c
c     Local:
c
...
c
      ...
      real*8 ra2_val(mxp,natoms,2)
      real*8 ra2_comp(mxp,natoms,3)
      real*8 wt(mxp)   ! On input : the partition function
                     ! On output: the non-zero weights
      real*8 wt2(mxp)  ! copy of the original Lebedev or Legendre weights
c
c...  Local variables
c
