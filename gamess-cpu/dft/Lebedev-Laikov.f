c Lebedev grids upt 131st order in 16 digit precision
c ===================================================
c
c The file Lebedev-Laikov.F implements a set of subroutines providing
c Lebedev-Laikov grids of order n=2m+1, where m=1,2,...,15, and 
c additionally grids of order n=6m+5, where m=5,6,...,21. The parameters
c ensure that angular integration of polynomials x**k * y**l * z**m, 
c where k+l+m <= 131 can be performed with a relative accuracy of 
c 2e-14 [1]. Note that the weights are normalised to add up to 1.0.
c
c For each order n a separate subroutine is provided named 
c LD<number of grid points>. The parameter P is an array for the
c cartesian components of each point, and the parameter W is an array 
c for the weights. The subroutines increase the integer parameter N by 
c the number of grid points generated. All these routines use the 
c subroutine gen_oh which takes care of the octahedral symmetry of the 
c grids.
c
c Christoph van Wuellen (Ruhr-Universitaet, Bochum, Germany) generated 
c the routines in Lebedev-Laikov.F by translating the original 
c C-routines kindly provided by Dmitri Laikov (Moscow State University,
c Moscow, Russia). We are in debt to Dmitri Laikov for giving us 
c permission to make these routines publically available.
c
c Huub van Dam
c Daresbury Laboratory, Daresbury, United Kingdom
c April, 2000
c 
c References
c ==========
...
c-----------------------------------------------------------------------
c
       subroutine LebedevLaikov(nang,p,w)
       implicit none
       integer maxpt
       parameter(maxpt=5810)
       real*8 p(3,maxpt), w(maxpt)
       integer nang
c
c      This subroutine provides an interface to the Lebedev grids
c      that can be used directly from GAMESS-UK in the same way the
c      Lebedev grids were used originally. 
c
c      nang: On entry the number of points requested.
c            On return the number of points actually created 
c            (this may actually be more than what was asked for so
c             make sure the sizes of p and w are adequate).
c
c      p: On return the Cartesian coordinates of the grid points.
c
c      w: On return the weights for the grid points. The weights will
c         be normalised to add up to 4*Pi.
c
       integer i
       real*8 pi4
c
...
c
       if (nang.le.6) then
          call ld0006(p,w,nang)
       else if (nang.le.14) then
          call ld0014(p,w,nang)
       else if (nang.le.26) then
          call ld0026(p,w,nang)
       else if (nang.le.38) then
          call ld0038(p,w,nang)
       else if (nang.le.50) then
          call ld0050(p,w,nang)
       else if (nang.le.74) then
          call ld0074(p,w,nang)
       else if (nang.le.86) then
          call ld0086(p,w,nang)
       else if (nang.le.110) then
          call ld0110(p,w,nang)
       else if (nang.le.146) then
          call ld0146(p,w,nang)
       else if (nang.le.170) then
          call ld0170(p,w,nang)
       else if (nang.le.194) then
          call ld0194(p,w,nang)
       else if (nang.le.230) then
          call ld0230(p,w,nang)
       else if (nang.le.266) then
          call ld0266(p,w,nang)
       else if (nang.le.302) then
          call ld0302(p,w,nang)
       else if (nang.le.350) then
          call ld0350(p,w,nang)
       else if (nang.le.434) then
          call ld0434(p,w,nang)
       else if (nang.le.590) then
          call ld0590(p,w,nang)
       else if (nang.le.770) then
          call ld0770(p,w,nang)
       else if (nang.le.974) then
          call ld0974(p,w,nang)
       else if (nang.le.1202) then
          call ld1202(p,w,nang)
       else if (nang.le.1454) then
          call ld1454(p,w,nang)
       else if (nang.le.1730) then
          call ld1730(p,w,nang)
       else if (nang.le.2030) then
          call ld2030(p,w,nang)
       else if (nang.le.2354) then
          call ld2354(p,w,nang)
       else if (nang.le.2702) then
          call ld2702(p,w,nang)
       else if (nang.le.3074) then
          call ld3074(p,w,nang)
       else if (nang.le.3470) then
          call ld3470(p,w,nang)
       else if (nang.le.3890) then
          call ld3890(p,w,nang)
       else if (nang.le.4334) then
          call ld4334(p,w,nang)
       else if (nang.le.4802) then
          call ld4802(p,w,nang)
       else if (nang.le.5294) then
          call ld5294(p,w,nang)
       else if (nang.le.5810) then
          call ld5810(p,w,nang)
       else 
          write(6,*)'Biggest Lebedev grid available has ',
     +                 maxpt,' points only.'
          write(6,*)'Number of requested was ',nang
          call caserr('*** LebedevLaikov: nang exceeds maxpt !!!')
       endif
c
       do i = 1, nang
          w(i) = w(i)*pi4
...
c-----------------------------------------------------------------------
c
       subroutine gen_oh(code, num, p, w, a, b, v)
       implicit none
       real*8 p(3,*),w(*)
       real*8 a,b,v
       integer code
...
cvw
       ...
       call caserr('*** GEN_OH: invalid subgrid code')
    1  continue
       a=1.0d0
       p(x,1) =  a
       p(y,1) =  0.0d0
...
chvd
       ...
       Call GEN_OH( 1, N, P(1,N), W(N), A, B, V)
       N=N-1
       RETURN
       END
       SUBROUTINE LD0014(P,W,N)
       IMPLICIT NONE
       real*8 P(3,  14)
       real*8 W(    14)
       INTEGER N
...
chvd
       ...
       Call GEN_OH( 3, N, P(1,N), W(N), A, B, V)
       N=N-1
       RETURN
       END
       SUBROUTINE LD0026(P,W,N)
       IMPLICIT NONE
       real*8 P(3,  26)
       real*8 W(    26)
       INTEGER N
...
chvd
       ...
       Call GEN_OH( 2, N, P(1,N), W(N), A, B, V)
       V=0.3214285714285714D-1
       Call GEN_OH( 3, N, P(1,N), W(N), A, B, V)
       N=N-1
       RETURN
       END
       SUBROUTINE LD0038(P,W,N)
       IMPLICIT NONE
       real*8 P(3,  38)
       real*8 W(    38)
       INTEGER N
...
chvd
       ...
       Call GEN_OH( 5, N, P(1,N), W(N), A, B, V)
       N=N-1
       RETURN
       END
       SUBROUTINE LD0050(P,W,N)
       IMPLICIT NONE
       real*8 P(3,  50)
       real*8 W(    50)
       INTEGER N
...
chvd
       ...
       Call GEN_OH( 4, N, P(1,N), W(N), A, B, V)
       N=N-1
       RETURN
       END
       SUBROUTINE LD0074(P,W,N)
       IMPLICIT NONE
       real*8 P(3,  74)
       real*8 W(    74)
       INTEGER N
...
chvd
       ...
       Call GEN_OH( 6, N, P(1,N), W(N), A, B, V)
       N=N-1
       RETURN
       END
       SUBROUTINE LD0170(P,W,N)
       IMPLICIT NONE
       real*8 P(3, 170)
       real*8 W(   170)
       INTEGER N
...
chvd
       ...
       END
c
      subroutine ver_dft_lebedevlaikov(s,r,d)
      character*80 source
      character*30 revision
      character*60 date
      character s*(*), r*(*), d*(*)
      data source /
     +     "$Source: /c/qcg/cvs/psh/GAMESS-UK/dft/Lebedev-Laikov.m,v $
     +     "/
      data revision /
     +     "$Revision: 5774 $"
     +      /
