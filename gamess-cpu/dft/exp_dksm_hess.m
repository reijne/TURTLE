...
c
      ...
      REAL adens(nao*(nao+1)/2)
      REAL bdens(nao*(nao+1)/2)
c
c     Out variables:
c
...
c     Out variables:
c
      REAL drhodb(mxp,2,3*n_active_atm)
      REAL dgrhodb(mxp,2,3,3*n_active_atm)
c
c     Local variables:
c
...
c
      ...
      REAL bfn_hess(mxp,nao,6)
      REAL grho(mxp,2,3)
      REAL drhodb(mxp,2,npert)    ! explicit derivatives only
      REAL dgrhodb(mxp,2,3,npert) ! explicit derivatives only
      REAL xc_vpt(mxp,2)
      REAL xc_dvpt(mxp,3)
      REAL xc_hpt(mxp,3)
      REAL xc_dhpt(mxp,12)
...
c     Out variables:
c
      REAL qa((nao+1)*nao/2,npert)
      REAL qb((nao+1)*nao/2,npert)
c
c     Local variables:
c
...
c
      REAL gagmn, gbgmn
      REAL gadbxgmn, gadbygmn, gadbzgmn
      REAL gbdbxgmn, gbdbygmn, gbdbzgmn
      REAL gaadbx, gabdbx, gbbdbx
      REAL gaadby, gabdby, gbbdby
      REAL gaadbz, gbbdbz, gabdbz
      REAL gagmndbx, gagmndby, gagmndbz
      REAL gbgmndbx, gbgmndby, gbgmndbz
c
      REAL gadbgmn(3)
      REAL gaadb(3)
c     REAL gagmndb(3)
...
c
      ...
      REAL bfn_hess(mxp,nao,6)
      REAL grho(mxp,2,3)
      REAL drhodb(mxp,2,3*n_active_atm)    ! explicit derivatives only
      REAL dgrhodb(mxp,2,3,3*n_active_atm) ! explicit derivatives only
      REAL xc_vpt(mxp,2)
      REAL xc_dvpt(mxp,3)
      REAL xc_hpt(mxp,3)
      REAL xc_dhpt(mxp,12)
...
c     Outputs
c
      REAL qa(nvec*(nvec+1)/2,npert)
c
c     Workspace
c
      REAL dmo(npts,npert)
c     REAL ddmo(npts,6,ngridcent)
      REAL drho(npts,npert)
      REAL tmo(npts,nvec)
c     REAL ddrho(npts,npert*(npert+1)/2)
c
c     Local variables
c
      integer i, j
...
c
      ...
      REAL bfng_val(mxp,nao,3)
      REAL bfn_hess(mxp,nao,6)
c     REAL bfn_3rd(mxp,nao,10)
      REAL avec(nao,nvec)
      REAL grho(mxp,2,3)
c
c     Outputs
c
      REAL qa(nao*(nao+1)/2,npert)
c
c     Workspace
c
      REAL dmo(npts,npert)
      REAL dgmo(npts,npert,3)
c     REAL ddmo(npts,6,ngridcent)
c     REAL ddgmo(npts,10,ngridcent)
      REAL drho(npts,npert)
c     REAL ddrho(npts,npert*(npert+1)/2)
      REAL dgrho(npts,npert,3)
c     REAL ddgrho(npts,npert*(npert+1)/2,3)
      REAL gmorho(npts,nvec)
      REAL grdgmo(npts)
      REAL t1(npts)
      REAL t2(npts)
      REAL t3(npts)
      REAL t4(npts)
      REAL tptmo(npts,nvec)
c
c     Local variables
c
      integer i, j
...
c
      ...
      integer latm
      REAL wt(npts)
      REAL gwt(3,mxp,ngridcent)
      REAL xc_vpt(mxp,2)
      REAL xc_hpt(mxp,3)
      REAL amo_val(npts,nvec)
      REAL bfng_val(mxp,nao,3)
c     REAL bfn_hess(mxp,nao,6)
      REAL avec(nao,nvec)
c
c     Outputs
c
...
c
      ...
      integer mxp
      REAL wt(mxp)
      REAL gwt(3,mxp,ngridcentres)
      REAL ggwt(mxp,3*ngridcentres,3*ngridcentres)
      REAL grho(mxp,2,3)
      REAL drho(mxp,2,npert)     ! explicit derivatives only
      REAL dgrho(mxp,2,3,npert)  ! explicit derivatives only
      REAL ddrhoa(npts,9)        ! explicit derivatives only
      REAL ddgrhoa(npts,3,9)     ! explicit derivatives only
      REAL ddrhob(npts,9)        ! explicit derivatives only
      REAL ddgrhob(npts,3,9)     ! explicit derivatives only
      REAL bfn_val(mxp,nao)
      REAL bfng_val(mxp,nao,3)
      REAL bfn_hess(mxp,nao,6)
...
c
      ...
      REAL ggwt(mxp,3*num_near_atoms,3*num_near_atoms)
      REAL grho(mxp,2,3)
      REAL bfn_val(mxp,nao)
      REAL bfng_val(mxp,nao,3)
      REAL bfn_hess(mxp,nao,6)
...
c
      REAL drho(mxp,2,3*n_active_atm)
      REAL dgrho(mxp,2,3,3*n_active_atm)
      REAL ddrhoa(npts,9)
      REAL ddrhob(npts,9)
      REAL ddgrhoa(npts,3,9)
      REAL ddgrhob(npts,3,9)
c
c     ))) explicit derivatives only 
c
      REAL xc_ept(mxp)
...
c
      REAL avec(nao,naocc)
      REAL amo_val(npts,naocc)
      REAL amo_grad(npts,naocc,3)
      REAL grho(mxp,2,3)
c
c     Outputs
c
...
c
      REAL drho(npts,npert)
      REAL dgrho(npts,3,npert)
      REAL dgaa(npts,npert)
      REAL ddrhoa(npts,9)
      REAL ddgrhoa(npts,3,9)
      REAL bdmo(npts,3)
      REAL admo(npts,3)
...
c
      REAL wt(mxp)
      REAL gwt(3,mxp,num_near_atoms)
      REAL g2wt(mxp,3*num_near_atoms,3*num_near_atoms)
c
      REAL bfn_val(mxp,nao)
      REAL bfng_val(mxp,nao,3)
      REAL bfn_hess(mxp,nao,6)
...
c     Workspave
c
      REAL drho(npts,3*n_active_atm)
      REAL dgrho(npts,3,3*n_active_atm)
      REAL dgaa(npts,3*n_active_atm)
      REAL ddrhoa(npts,9)
      REAL ddgrhoa(npts,3,9)
      REAL bdmo(npts,3)
      REAL admo(npts,3)
