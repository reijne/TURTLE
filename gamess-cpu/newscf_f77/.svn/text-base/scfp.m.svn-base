...
c
      ...
      if (nt.gt.1 .or. idadap .ne. 1) then
       call caserr('disable symmetry and adaption to use newscf')
      endif

      if (oharm) then
       call caserr('disable harmonic to use newscf')
      endif
...
c
     ...
     &           'newscf only available for (closed shell) rhf and uhf')
         ...
         call newscf(q,ouhf,odscf,en,ehf,etot,sz,s2,ek,vir)
         goto 160
      endif
c
c ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
